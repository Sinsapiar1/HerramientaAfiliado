...
`;o.textContent=a+o.textContent}},validateApiKey:e=>!e||e.trim().length===0?{valid:!1,message:"API Key vacía"}:e.length<20?{valid:!1,message:"API Key muy corta"}:e.startsWith("AIza")?{valid:!0,message:"API Key válida"}:{valid:!1,message:"Formato de API Key inválido para Google AI Studio"},showStatus:(e,t)=>{const o=document.getElementById("statusDiv"),n={success:"✅",error:"❌",warning:"⚠️",info:"ℹ️"};o.innerHTML=`<div class="status ${t}">${n[t]} ${e}</div>`,c.log(`Estado: ${t}`,e)},updateLoadingStep:e=>{document.querySelectorAll(".step").forEach((t,o)=>{o<e?t.classList.add("active"):t.classList.remove("active")})}},A={testConnection:async()=>{if(!m.apiKey)return c.showStatus("Primero guarda tu API Key","error"),!1;const e=document.getElementById("testBtn"),t=e.textContent;e.textContent="🧪 Probando...",e.disabled=!0;try{c.log("Iniciando test de API...");const n=await A.callGemini('Responde solo con "OK" si recibes este mensaje.');return n&&n.toLowerCase().includes("ok")?(c.showStatus("API funcionando correctamente","success"),document.getElementById("debugApiStatus").textContent="Funcionando ✅",c.log("Test de API exitoso",n),!0):(c.showStatus("API responde pero formato inesperado","warning"),document.getElementById("debugApiStatus").textContent="Respuesta inesperada ⚠️",c.log("Test de API - respuesta inesperada",n),!1)}catch(o){return c.showStatus(`Error en API: ${o.message}`,"error"),document.getElementById("debugApiStatus").textContent="Error ❌",c.log("Test de API falló",o,"error"),!1}finally{e.textContent=t,e.disabled=!1}},callGemini:async e=>{if(!m.apiKey)throw new Error("API Key no configurada");const t=`${x.api.baseUrl}?key=${m.apiKey}`,o={contents:[{parts:[{text:e}]}],generationConfig:{temperature:x.api.temperature,maxOutputTokens:x.api.maxTokens,topP:.8,topK:40}};c.log("Enviando petición a Gemini API...",{promptLength:e.length});const n=await fetch(t,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)});if(!n.ok){const r=await n.text();c.log("Error en respuesta de API",{status:n.status,error:r},"error");const s={401:"API Key inválida o sin permisos",429:"Límite de requests excedido. Intenta en unos minutos",400:"Request inválido. Verifica la configuración",403:"Acceso denegado. Verifica tu API Key"};throw new Error(s[n.status]||`Error ${n.status}: ${r}`)}const a=await n.json();if(c.log("Respuesta recibida de API",a),!a.candidates||!a.candidates[0]||!a.candidates[0].content)throw new Error("Respuesta de API incompleta o bloqueada por filtros de seguridad");const i=a.candidates[0].content.parts[0].text;return c.log("Texto de respuesta extraído",{length:i.length}),i}},ae={generateAffilatePrompt:e=>{const{nicho:t,publico:o,rangoPrecios:n,tipoProducto:a,canalPrincipal:i,experiencia:r,keywords:s,presupuestoAds:l,roiObjetivo:d,breakEvenTime:u,tipoConversion:p,dispositivoTarget:g,mercadoGeo:v,analyzeCompetition:C,analyzeTrends:T,findAffiliates:F,analyzeKeywords:U,analyzeSeasonality:z,analyzeProfitability:b,analyzeConversion:G,analyzeFinancial:y,analyzeCompetitorIntel:Q,analyzeCustomerJourney:Oe,analyzeTrafficChannels:Z,analyzeFunnels:$e}=e,ee={bajo:"$10-$50",medio:"$50-$200",alto:"$200-$500","muy-alto":"$500+"}[n],N=l==="0"?"Sin presupuesto (Tráfico orgánico)":`$${l}+ mensual`,te=s?`
...
🎯 ANÁLISIS REQUERIDOS: ${[C&&"Competencia",T&&"Tendencias",z&&"Estacionalidad",G&&"Conversión",y&&"Financiero",Q&&"Intel Competitiva",Z&&"Canales Tráfico"].filter(Boolean).join(", ")}`;return`Actúa como CONSULTOR EXPERTO en marketing de afiliados especializado en ${t} para ${i} en ${v} con 15+ años detectando productos ganadores.
...
- Lenguaje que convierte en ${o.nicho}`,d=await A.callGemini(l);console.log("🎯 Generando avatar específico para el producto...");let u=null;try{window.AvatarSyncSystem?(u=await AvatarSyncSystem.generarAvatarEspecifico(o,a),console.log("✅ Avatar específico generado:",u.nombre)):console.log("⚠️ AvatarSyncSystem no disponible, continuando sin avatar específico")}catch(p){console.error("Error generando avatar específico:",p)}le(d,a,o,u),c.showStatus(`✅ Contenido inteligente ${u?"+ avatar específico":""} generado para ${a.length} tipos`,"success")}catch(o){console.error("Error:",o),c.showStatus(`❌ Error: ${o.message}`,"error")}finally{e.innerHTML=t,e.disabled=!1}}async function ce(){if(console.log("Generando avatar..."),!m.apiKey){alert("⚠️ Configura tu API Key primero");return}const e=document.getElementById("nicho").value.trim(),t=document.getElementById("publico").value.trim();if(!e||!t){alert("⚠️ Completa el nicho y público objetivo");return}const o=document.getElementById("generateAvatarBtn"),n=o.innerHTML;o.innerHTML="🔄 Creando...",o.disabled=!0;try{const a=`Actúa como PSICÓLOGO EXPERTO EN MARKETING con doctorado en comportamiento del consumidor.
...
Haz este avatar TAN específico que cualquier marketer pueda hablarle directamente y convertir 3-5x más.`,i=await A.callGemini(a);pe(i),c.showStatus("✅ Avatar creado exitosamente","success")}catch(a){console.error("Error:",a),c.showStatus(`❌ Error: ${a.message}`,"error")}finally{o.innerHTML=n,o.disabled=!1}}function le(e,t,o,n=null){let a=document.getElementById("contentResults");if(a||(a=document.createElement("div"),a.id="contentResults",a.className="content-results",document.querySelector(".main-content").appendChild(a)),a.innerHTML=`
...
`).filter(d=>d.trim()).map((d,u)=>`
    ...
    `,document.body.appendChild(t),setTimeout(()=>{t.style.opacity="0",setTimeout(()=>t.remove(),300)},3e3)}};function _(){document.querySelectorAll(".product-opportunity").forEach((e,t)=>{if(!e.querySelector(".validate-btn")){let o=e.querySelector(".product-actions");o||(o=document.createElement("div"),o.className="product-actions",e.appendChild(o));const n=document.createElement("button");n.className="btn btn-secondary validate-btn",n.innerHTML="🔍 Validar Oferta",n.dataset.productIndex=t,n.onclick=async function(){const a=m.productosDetectados[this.dataset.productIndex];this.disabled=!0,this.innerHTML="🔄 Validando...";try{const i=await $.validateOffer(a.nombre,document.getElementById("nicho").value);i&&$.displayValidation(i,a.nombre,e)}catch(i){console.error("Error validando:",i),alert("Error al validar. Intenta de nuevo.")}finally{this.disabled=!1,this.innerHTML="🔍 Validar Oferta"}},o.appendChild(n)}})}const L={spiedProducts:new Set,spyWinningAds:async(e,t,o)=>{if(!m.apiKey){alert("⚠️ Configura tu API Key primero");return}const n=`Actúa como EXPERTO EN FACEBOOK ADS LIBRARY y TIKTOK CREATIVE CENTER con acceso completo a todas las campañas activas.
...
`,o+=this.currentScenarios.recommendations||"No disponibles";const a=new Blob([o],{type:"text/plain"}),i=URL.createObjectURL(a),r=document.createElement("a");r.href=i,r.download=`profit-report-${Date.now()}.txt`,r.click(),URL.revokeObjectURL(i),alert("✅ Reporte exportado exitosamente")}catch(e){console.error("Error exportando reporte:",e),alert("❌ Error al exportar reporte")}},saveScenario:function(){if(!this.currentScenarios){alert("⚠️ No hay escenarios para guardar");return}try{const e=JSON.parse(localStorage.getItem("saved_scenarios")||"[]");e.push({date:new Date().toISOString(),product:this.currentProduct.nombre,scenarios:this.currentScenarios,config:{budget:document.getElementById("calcBudget").value,channel:document.getElementById("calcChannel").value,days:document.getElementById("calcDays").value,market:document.getElementById("calcMarket").value}}),localStorage.setItem("saved_scenarios",JSON.stringify(e)),alert("✅ Escenario guardado exitosamente")}catch(e){console.error("Error guardando escenario:",e),alert("❌ Error al guardar escenario")}}};function Y(){console.log("💰 Agregando botones de Profit Calculator..."),document.querySelectorAll(".product-opportunity").forEach((e,t)=>{if(!e.querySelector(".profit-calc-btn")){const o=e.querySelector(".product-actions")||e.querySelector(".spy-btn")?.parentElement||e,n=document.createElement("button");n.className="btn btn-secondary profit-calc-btn",n.innerHTML="💰 Calcular Profit",n.style.marginTop="10px",n.style.marginLeft="10px",n.onclick=()=>{if(m.productosDetectados&&m.productosDetectados[t]){const a=m.productosDetectados[t];H.open(a,t)}else alert("⚠️ Producto no encontrado")},e.querySelector(".spy-btn")?e.querySelector(".spy-btn").after(n):e.querySelector(".validate-btn")?e.querySelector(".validate-btn").after(n):o.appendChild(n)}}),console.log("✅ Botones de Profit Calculator agregados")}if(typeof D<"u"){const e=D;D=function(){try{e(),setTimeout(Y,100)}catch(t){console.error("Error en addSpyButtons:",t)}}}const I={generators:{facebook:(e,t)=>{const o=e.painPoints||"",n=e.emociones||"",a=e.triggers||"",i=e.descripcion||"",r=e.estacionalidad||"",s=e.horarioOptimo||"",l=e.competenciaNivel||"",d=e.timingOptimo||"",u=e.estrategia||"",p=o.split(/[,.]/).filter(b=>b.trim()),g=n.split(/[,.]/).filter(b=>b.trim()),v=a.split(/[,.]/).filter(b=>b.trim()),C=p.slice(0,3).map((b,G)=>{const y=b.trim().toLowerCase();return y.includes("falta de tiempo")?"✅ Resultados en solo 15 minutos al día":y.includes("dificultad")?"✅ Método simple paso a paso que cualquiera puede seguir":y.includes("peso")||y.includes("grasa")?"✅ Pierde hasta 2 kilos por semana sin pasar hambre":y.includes("energía")?"✅ Energía ilimitada desde el primer día":y.includes("dinero")||y.includes("caro")?"✅ Inversión mínima con resultados máximos":y.includes("motivación")?"✅ Sistema que te mantiene motivado todos los días":`✅ ${b.trim().replace(/no poder|no lograr|falta de|sin/gi,"Lograrás").replace(/dificultad para|problema con/gi,"Dominarás")}`});C.length===0&&C.push("✅ Resultados visibles desde la primera semana","✅ Método probado por miles de personas","✅ Garantía de satisfacción del 100%");const T=g[0]||"frustración",F=v[0]||"necesidad de cambio",U=[`😱 ¿${T.charAt(0).toUpperCase()+T.slice(1)}? ${e.nombre} es la solución que buscabas`,`🔥 "${p[0]||"Este problema"}" - Si esto te suena familiar, necesitas ${e.nombre}`,`⚠️ ATENCIÓN: ${e.nombre} con ${e.comision||"40% descuento"} (Solo hoy)`,`💥 ${F.charAt(0).toUpperCase()+F.slice(1)}? Descubre cómo ${e.nombre} está cambiando vidas`,`🎯 Por fin: La solución definitiva para ${p[0]||t} está aquí`];return`${U[Math.floor(Math.random()*U.length)]}
...
IMPORTANTE: Devuelve SOLO el copy optimizado, sin explicaciones.`;try{return await A.callGemini(n)}catch(a){return console.error("Error generando copy con IA:",a),I.generators[o](e,t)}},generateABVariations:async(e,t,o)=>{const n=`
...
Por ahora, usa los hooks y ángulos proporcionados para crear tus propias variantes.`)),e.target.matches(".download-template-btn")){e.preventDefault();const t=e.target.dataset.spyId,o=document.getElementById(t);if(o){const n=o.innerText,a=new Blob([n],{type:"text/plain"}),i=URL.createObjectURL(a),r=document.createElement("a");r.href=i,r.download=`ad-template-${Date.now()}.txt`,r.click(),URL.revokeObjectURL(i),I.showNotification("✅ ¡Template descargado!","success")}}});setInterval(()=>{try{const e=document.querySelectorAll(".product-opportunity");e.length>0&&m.productosDetectados.length>0&&(Array.from(e).some(a=>!a.querySelector(".profit-calc-btn"))&&Y(),Array.from(e).some(a=>!a.querySelector(".template-buttons"))&&I.addTemplateButtons(),Array.from(e).some(a=>!a.querySelector(".validate-btn"))&&typeof _<"u"&&_())}catch{}},5e3);console.log("✅ MarketInsight Pro cargado completamente");function Se(){Te(),setTimeout(()=>{const e=document.querySelector(".advanced-config-section");e&&(e.style.display="none",console.log("🔧 Configuración avanzada oculta temporalmente"))},500)}function Te(){const e=document.getElementById("themeToggle"),t=document.body;if(!e)return;const o=localStorage.getItem("theme")||"dark";t.setAttribute("data-theme",o),e.addEventListener("click",()=>{const a=t.getAttribute("data-theme")==="dark"?"light":"dark";t.setAttribute("data-theme",a),localStorage.setItem("theme",a),t.style.transition="all 0.3s ease",setTimeout(()=>{t.style.transition=""},300),c.log(`🎨 Tema cambiado a: ${a}`)})}typeof h<"u"&&(h.generateAdditionalProductsAdvanced=function(e){const t=window.advancedConfig||{productCount:3,minScore:70,activeFilters:["BAJO","MEDIO","ALTO"]},o=t.productCount-e;if(o<=0)return[];const n=[],a=document.getElementById("nicho")?.value||"marketing",i=document.getElementById("publico")?.value||"audiencia",r=[`Curso Avanzado de ${a}`,`Masterclass Completa de ${a}`,`Sistema Premium de ${a}`,`Guía Definitiva de ${a}`,`Entrenamiento VIP de ${a}`,`Blueprint de ${a}`,`Manual Profesional de ${a}`,`Certificación en ${a}`,`Mentoring de ${a}`,`Toolkit de ${a}`];for(let s=0;s<o&&s<r.length;s++){const l=Math.floor(Math.random()*(95-t.minScore))+t.minScore,d=t.activeFilters[Math.floor(Math.random()*t.activeFilters.length)];n.push({nombre:r[s],precio:h.extractRandomPrice(),comision:h.extractRandomCommission(),score:l,descripcion:`Producto profesional de ${a} dirigido a ${i}`,painPoints:[`Falta de conocimiento en ${a}`,"Necesidad de resultados rápidos","Búsqueda de métodos probados"],emociones:["Frustración","Esperanza","Determinación"],triggers:["Exclusividad","Resultados garantizados","Soporte premium"],competencia:d,networks:["ClickBank","ShareASale","CJ Affiliate"],tips:[`Enfocarse en ${i} específicamente`,"Usar testimonios reales","Crear urgencia con tiempo limitado"]})}return c.log(`✅ Generados ${n.length} productos adicionales con score mínimo ${t.minScore}`),n});document.addEventListener("DOMContentLoaded",function(){setTimeout(()=>{Se(),c.log("✅ Funcionalidades avanzadas inicializadas")},1e3)});console.log("✅ Funcionalidades profesionales cargadas");typeof window<"u"&&(window.APIManager=A,window.AppState=m,window.Utils=c);console.log("MarketInsight Pro (Vite + TS) initialized");const q={gemini:{keyLink:"https://aistudio.google.com/app/apikey",request:async()=>{throw new Error("Gemini request debe ser manejada por la implementación original")}},openai:{keyLink:"https://platform.openai.com/account/api-keys",request:async(e,t)=>{const o=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify({model:"gpt-3.5-turbo",messages:[{role:"user",content:e}],temperature:.7,max_tokens:4e3})});if(!o.ok){const a=await o.text();throw new Error(`OpenAI error ${o.status}: ${a}`)}return(await o.json()).choices?.[0]?.message?.content||""}},together:{keyLink:"https://app.together.xyz/settings/api-keys",request:async(e,t)=>{const o=await fetch("https://api.together.xyz/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify({model:"mistral-7b-instruct",messages:[{role:"user",content:e}],temperature:.7,max_tokens:4e3})});if(!o.ok){const a=await o.text();throw new Error(`Together.ai error ${o.status}: ${a}`)}const n=await o.json();return(n.text||n.response||n.generations?.[0]?.text)??""}},cohere:{keyLink:"https://dashboard.cohere.com/api-keys",request:async(e,t)=>{const o=await fetch("https://api.cohere.ai/v1/chat",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify({model:"command-r",message:e,temperature:.7,max_tokens:4e3})});if(!o.ok){const a=await o.text();throw new Error(`Cohere error ${o.status}: ${a}`)}const n=await o.json();return(n.text||n.response||n.generations?.[0]?.text)??""}}};function K(){const e=document.getElementById("apiProvider"),t=document.getElementById("getKeyLink");if(!e)return;const o=localStorage.getItem("aiProvider")||"gemini";e.value=o,window.AppState.apiProvider=o;const n=()=>{const a=e.value;window.AppState.apiProvider=a,localStorage.setItem("aiProvider",a),t&&(t.href=q[a].keyLink,t.textContent=`Obtener API Key (${q[a].name})`)};e.addEventListener("change",n),n()}function X(){const e=window;if(!e.APIManager)return;const t=e.APIManager.callGemini.bind(e.APIManager);e.APIManager.callGemini=async o=>{const n=e.AppState?.apiProvider||"gemini";if(n==="gemini")return t(o);const a=q[n];if(!a)throw new Error(`Proveedor no soportado: ${n}`);return await a.request(o,e.AppState.apiKey)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{K(),X(),J()}):(K(),X(),J());function J(){const e=window;e.Utils&&(e.Utils.validateApiKey=t=>{const o=e.AppState?.apiProvider||"gemini";return!t||t.trim().length===0?{valid:!1,message:"API Key vacía"}:o==="gemini"?t.length<20||!t.startsWith("AIza")?{valid:!1,message:"Formato de API Key inválido para Google AI Studio"}:{valid:!0,message:"API Key válida"}:t.length<20?{valid:!1,message:"API Key muy corta"}:{valid:!0,message:"API Key válida"}})}
