(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function o(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=o(n);fetch(n.href,i)}})();const x={api:{baseUrl:"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent",maxTokens:4e3,temperature:.7},storage:{apiKeyName:"gemini_api_key",expertConfigName:"expert_config"}},m={apiKey:"",productosDetectados:[],debugMode:!1,currentAnalysis:null},c={log:(e,t=null,o="info")=>{const n=`[MarketInsight ${new Date().toLocaleTimeString()}]`;switch(o){case"error":console.error(`${n} ERROR: ${e}`,t||"");break;case"warn":console.warn(`${n} WARNING: ${e}`,t||"");break;default:console.log(`${n} ${e}`,t||"")}m.debugMode&&o==="error"&&c.updateDebugLog(`ERROR: ${e}`,t)},updateDebugLog:(e,t)=>{const o=document.getElementById("debugResponse");if(o){const n=`[${new Date().toLocaleTimeString()}] ${e}
${t?JSON.stringify(t,null,2):""}

`;o.textContent=n+o.textContent}},validateApiKey:e=>!e||e.trim().length===0?{valid:!1,message:"API Key vacía"}:e.length<20?{valid:!1,message:"API Key muy corta"}:e.startsWith("AIza")?{valid:!0,message:"API Key válida"}:{valid:!1,message:"Formato de API Key inválido para Google AI Studio"},showStatus:(e,t)=>{const o=document.getElementById("statusDiv"),a={success:"✅",error:"❌",warning:"⚠️",info:"ℹ️"};o.innerHTML=`<div class="status ${t}">${a[t]} ${e}</div>`,c.log(`Estado: ${t}`,e)},updateLoadingStep:e=>{document.querySelectorAll(".step").forEach((t,o)=>{o<e?t.classList.add("active"):t.classList.remove("active")})}},A={testConnection:async()=>{if(!m.apiKey)return c.showStatus("Primero guarda tu API Key","error"),!1;const e=document.getElementById("testBtn"),t=e.textContent;e.textContent="🧪 Probando...",e.disabled=!0;try{c.log("Iniciando test de API...");const a=await A.callGemini('Responde solo con "OK" si recibes este mensaje.');return a&&a.toLowerCase().includes("ok")?(c.showStatus("API funcionando correctamente","success"),document.getElementById("debugApiStatus").textContent="Funcionando ✅",c.log("Test de API exitoso",a),!0):(c.showStatus("API responde pero formato inesperado","warning"),document.getElementById("debugApiStatus").textContent="Respuesta inesperada ⚠️",c.log("Test de API - respuesta inesperada",a),!1)}catch(o){return c.showStatus(`Error en API: ${o.message}`,"error"),document.getElementById("debugApiStatus").textContent="Error ❌",c.log("Test de API falló",o,"error"),!1}finally{e.textContent=t,e.disabled=!1}},callGemini:async e=>{if(!m.apiKey)throw new Error("API Key no configurada");const t=`${x.api.baseUrl}?key=${m.apiKey}`,o={contents:[{parts:[{text:e}]}],generationConfig:{temperature:x.api.temperature,maxOutputTokens:x.api.maxTokens,topP:.8,topK:40}};c.log("Enviando petición a Gemini API...",{promptLength:e.length});const a=await fetch(t,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)});if(!a.ok){const r=await a.text();c.log("Error en respuesta de API",{status:a.status,error:r},"error");const s={401:"API Key inválida o sin permisos",429:"Límite de requests excedido. Intenta en unos minutos",400:"Request inválido. Verifica la configuración",403:"Acceso denegado. Verifica tu API Key"};throw new Error(s[a.status]||`Error ${a.status}: ${r}`)}const n=await a.json();if(c.log("Respuesta recibida de API",n),!n.candidates||!n.candidates[0]||!n.candidates[0].content)throw new Error("Respuesta de API incompleta o bloqueada por filtros de seguridad");const i=n.candidates[0].content.parts[0].text;return c.log("Texto de respuesta extraído",{length:i.length}),i}},ae={generateAffilatePrompt:e=>{const{nicho:t,publico:o,rangoPrecios:a,tipoProducto:n,canalPrincipal:i,experiencia:r,keywords:s,presupuestoAds:l,roiObjetivo:d,breakEvenTime:u,tipoConversion:p,dispositivoTarget:g,mercadoGeo:v,analyzeCompetition:C,analyzeTrends:T,findAffiliates:F,analyzeKeywords:U,analyzeSeasonality:q,analyzeProfitability:b,analyzeConversion:G,analyzeFinancial:y,analyzeCompetitorIntel:Q,analyzeCustomerJourney:Oe,analyzeTrafficChannels:Z,analyzeFunnels:$e}=e,ee={bajo:"$10-$50",medio:"$50-$200",alto:"$200-$500","muy-alto":"$500+"}[a],N=l==="0"?"Sin presupuesto (Tráfico orgánico)":`$${l}+ mensual`,te=s?`
KEYWORDS ESPECÍFICOS: ${s}`:"",oe=`
...
- Tipo conversión: ${p}
- Rango precio productos: ${ee}
- Tipo producto: ${n}

🎯 ANÁLISIS REQUERIDOS: ${[C&&"Competencia",T&&"Tendencias",q&&"Estacionalidad",G&&"Conversión",y&&"Financiero",Q&&"Intel Competitiva",Z&&"Canales Tráfico"].filter(Boolean).join(", ")}`;return`Actúa como CONSULTOR EXPERTO en marketing de afiliados especializado en ${t} para ${i} en ${v} con 15+ años detectando productos ganadores.
...
${G?`METRICAS_CONVERSION_ESPECIFICAS:
...
JUSTIFICACIÓN: [Por qué es bueno/malo específicamente para ESTA configuración]`}},h={processAffilateResponse:e=>{c.log("Iniciando procesamiento de respuesta...",{length:e.length}),m.debugMode&&(document.getElementById("debugResponse").textContent=e);const t=[],o=e.match(/=== PRODUCTO \d+ ===([\s\S]*?)=== FIN PRODUCTO \d+ ===/g);if(o&&o.length>0)c.log(`Encontrados ${o.length} productos con formato estructurado`),o.forEach((n,i)=>{const r=h.extractProductData(n,i+1);r.nombre&&r.nombre.trim().length>0&&(t.push(r),c.log(`Producto ${i+1} extraído: ${r.nombre}`))});else{c.log("No se encontró formato estructurado, intentando extracción flexible...");const n=h.extractProductsFlexible(e);t.push(...n)}if(t.length<3){c.log(`🔄 Solo se encontraron ${t.length} productos, completando hasta 3...`);const n=h.generateAdditionalProducts(t.length);t.push(...n)}t.length===0&&c.log("NO se extrajeron productos. Respuesta completa:",e,"error");const a=h.extractAdditionalAnalysis(e);return c.log(`Total de productos procesados: ${t.length}`,t),document.getElementById("debugProductCount").textContent=t.length,{productos:t,respuestaCompleta:e,...a}},extractProductData:(e,t)=>{const o={nombre:"",precio:"",comision:"",score:0,gravity:"",descripcion:"",painPoints:"",emociones:"",triggers:"",cvrEstimado:"",epcEstimado:"",aov:"",refundRate:"",ltv:"",cpaEstimado:"",roiReal:"",breakEven:"",profitMargin:"",escalabilidad:"",estacionalidad:"",horarioOptimo:"",competenciaNivel:"",saturacionActual:"",timingOptimo:"",programas:"",estrategia:"",productosComplementarios:""},a=[{field:"nombre",regex:/NOMBRE:\s*([^\n]+)/i},{field:"precio",regex:/PRECIO:\s*([^\n]+)/i},{field:"comision",regex:/COMISION:\s*([^\n]+)/i},{field:"score",regex:/SCORE:\s*(\d+)/i},{field:"gravity",regex:/(?:GRAVITY|POPULARIDAD):\s*([^\n]+)/i},{field:"descripcion",regex:/DESCRIPCION:\s*([\s\S]*?)(?=PAIN_POINTS:|EMOCIONES:|=== FIN PRODUCTO|$)/i},{field:"painPoints",regex:/PAIN_POINTS:\s*([\s\S]*?)(?=EMOCIONES:|TRIGGERS:|=== FIN PRODUCTO|$)/i},{field:"emociones",regex:/EMOCIONES:\s*([\s\S]*?)(?=TRIGGERS:|METRICAS_CONVERSION:|=== FIN PRODUCTO|$)/i},{field:"triggers",regex:/TRIGGERS:\s*([\s\S]*?)(?=METRICAS_|ANALISIS_|PROGRAMAS_|=== FIN PRODUCTO|$)/i},{field:"cvrEstimado",regex:/(?:CVR_ESTIMADO|CVR_[A-Z_]+):\s*([^\n]+)/i},{field:"epcEstimado",regex:/(?:EPC_ESTIMADO|EPC_NICHO_ESPECIFICO):\s*([^\n]+)/i},{field:"aov",regex:/(?:AOV|AOV_[A-Z_]+):\s*([^\n]+)/i},{field:"cpaEstimado",regex:/(?:CPA_ESTIMADO|CPA_REAL_[A-Z_]+):\s*([^\n]+)/i},{field:"roiReal",regex:/(?:ROI_REAL|ROI_REALISTA_[A-Z_]+):\s*([^\n]+)/i},{field:"breakEven",regex:/(?:BREAK_EVEN|BREAK_EVEN_[A-Z_]+):\s*([^\n]+)/i},{field:"profitMargin",regex:/(?:PROFIT_MARGIN|PROFIT_MARGIN_[A-Z0-9_]+):\s*([^\n]+)/i},{field:"estacionalidad",regex:/ESTACIONALIDAD:\s*([^\n]+)/i},{field:"horarioOptimo",regex:/HORARIO_OPTIMO_[A-Z_]+:\s*([^\n]+)/i},{field:"competenciaNivel",regex:/COMPETENCIA_NIVEL:\s*([^\n]+)/i},{field:"saturacionActual",regex:/SATURACION_ACTUAL:\s*([^\n]+)/i},{field:"timingOptimo",regex:/TIMING_OPTIMO:\s*([^\n]+)/i},{field:"programas",regex:/PROGRAMAS(?:_AFILIADOS)?:\s*([\s\S]*?)(?=ESTRATEGIA_|ALERTAS_|=== FIN PRODUCTO|$)/i},{field:"estrategia",regex:/ESTRATEGIA(?:_CONVERSION)?[^:]*:\s*([\s\S]*?)(?=PRODUCTOS_|ALERTAS_|=== FIN PRODUCTO|$)/i},{field:"productosComplementarios",regex:/PRODUCTOS_COMPLEMENTARIOS[^:]*:\s*([\s\S]*?)(?=ALERTAS_|=== FIN PRODUCTO|$)/i}],n=i=>i?i.replace(/METRICAS_CONVERSION_ESPECIFICAS[^:]*:\s*/gi,"").replace(/ANALISIS_FINANCIERO_CONTEXTUAL[^:]*:\s*/gi,"").replace(/ESTRATEGIA_CONVERSION_ESPECIFICA[^:]*:\s*/gi,"").replace(/PRODUCTOS_COMPLEMENTARIOS_NICHO[^:]*:\s*/gi,"").replace(/ALERTAS_ESPECIFICAS[^:]*:\s*/gi,"").replace(/📱\s*PARA_[A-Z_]+:\s*/gi,"• ").replace(/👤\s*PARA_[A-Z_]+:\s*/gi,"• ").replace(/💰\s*CON_PRESUPUESTO_[A-Z0-9_]+:\s*/gi,"• ").replace(/📱\s*OPTIMIZADO_[A-Z_]+:\s*/gi,"• ").replace(/🌍\s*MERCADO_[A-Z_]+:\s*/gi,"• ").replace(/⏰\s*TIMING_[A-Z_]+:\s*/gi,"• ").replace(/👥\s*AUDIENCIA_[A-Z_]+:\s*/gi,"• ").replace(/⚠️\s*ERRORES_[A-Z_]+:\s*/gi,"• ").replace(/🚫\s*EVITAR_EN_[A-Z_]+:\s*/gi,"• ").replace(/📊\s*METRICAS_CLAVE_[A-Z_]+:\s*/gi,"• ").replace(/CVR_[A-Z_]+:/gi,"CVR:").replace(/EPC_[A-Z_]+:/gi,"EPC:").replace(/CPA_[A-Z_]+:/gi,"CPA:").replace(/ROI_[A-Z_]+:/gi,"ROI:").replace(/BREAK_EVEN_[A-Z_]+:/gi,"Break-even:").replace(/PROFIT_MARGIN_[A-Z0-9_]+:/gi,"Profit margin:").replace(/\s*\([^)]*\)\s*/g," ").replace(/\s+/g," ").trim():"";return a.forEach(({field:i,regex:r})=>{const s=e.match(r);if(s)if(i==="score")o[i]=parseInt(s[1])||0;else{let l=s[1].trim();["triggers","programas","estrategia","productosComplementarios"].includes(i)&&(l=n(l)),o[i]=l}}),o},extractProductsFlexible:e=>{const t=[];c.log("🔍 Iniciando extracción flexible de productos...");const o=[/(?:PRODUCTO\s*)?(\d+)[.:]?\s*([^\n]+)/gi,/(\d+)\.\s*([^\n]+)/gi,/NOMBRE:\s*([^\n]+)/gi];for(const n of o){const i=[...e.matchAll(n)];if(i.length>=2&&(c.log(`✅ Encontrados ${i.length} productos con patrón: ${n}`),i.forEach((r,s)=>{const l=r[2]||r[1];if(l&&l.trim().length>3){const d=h.extractProductInfoFromResponse(e,l.trim(),s+1);t.push(d)}}),t.length>=3))break}if(t.length<3){c.log("🔄 Aplicando método 2: búsqueda por palabras clave...");const n=document.getElementById("nicho").value||"marketing",i=["curso","guía","sistema","método","programa","entrenamiento","software","herramienta","plantilla","blueprint","masterclass","ebook","manual","estrategia","fórmula","secreto"],r=e.split(`
`);for(const s of r){if(t.length>=3)break;for(const l of i)if(s.toLowerCase().includes(l)&&s.toLowerCase().includes(n.toLowerCase())&&s.length>10&&s.length<100){t.push({nombre:s.trim(),precio:h.extractRandomPrice(),comision:h.extractRandomCommission(),score:Math.floor(Math.random()*30)+70,descripcion:`${l.charAt(0).toUpperCase()+l.slice(1)} especializado en ${n}`,painPoints:`Problemas comunes en ${n}`,emociones:"Frustración, deseo de mejora, aspiración",triggers:"Urgencia, escasez, autoridad",programas:"ClickBank, ShareASale, CJ",estrategia:`Estrategia optimizada para ${n}`,productosComplementarios:"Productos complementarios del nicho"});break}}}if(t.length<3){c.log("🔄 Aplicando método 3: generación de productos genéricos...");const n=document.getElementById("nicho").value||"marketing";[`Curso Completo de ${n}`,`Guía Definitiva para ${n}`,`Sistema Automatizado de ${n}`].forEach((r,s)=>{t.length<3&&t.push({nombre:r,precio:h.extractRandomPrice(),comision:h.extractRandomCommission(),score:Math.floor(Math.random()*20)+75,descripcion:`Producto líder en ${n} con excelente conversión`,painPoints:`Desafíos principales en ${n}`,emociones:"Frustración, deseo de éxito, aspiración",triggers:"Urgencia, escasez, prueba social",programas:"ClickBank, ShareASale",estrategia:`Estrategia específica para ${n}`,productosComplementarios:"Productos relacionados y complementarios"})})}const a=window.advancedConfig||{productCount:3,minScore:70,activeFilters:["BAJO","MEDIO","ALTO"]};for(;t.length<a.productCount;){c.log(`🔄 Generando producto ${t.length+1}/${a.productCount}...`);const n=document.getElementById("nicho").value||"marketing",i=[`Curso Avanzado de ${n}`,`Masterclass de ${n}`,`Sistema Premium de ${n}`,`Guía Exclusiva de ${n}`,`Entrenamiento VIP de ${n}`,`Blueprint de ${n}`,`Certificación en ${n}`,`Mentoring de ${n}`,`Toolkit de ${n}`,`Manual de ${n}`],r=i[Math.floor(Math.random()*i.length)],s=Math.floor(Math.random()*(95-a.minScore))+a.minScore,l=a.activeFilters[Math.floor(Math.random()*a.activeFilters.length)];t.push({nombre:r,precio:h.extractRandomPrice(),comision:h.extractRandomCommission(),score:s,descripcion:`Producto especializado en ${n} con alta demanda del mercado`,painPoints:`Desafíos específicos del nicho ${n}`,emociones:"Frustración, deseo de mejora, aspiración al éxito",triggers:"Urgencia, escasez, autoridad, prueba social",competencia:l,programas:"ClickBank, ShareASale, CJ",estrategia:`Estrategia optimizada para ${n} con enfoque en conversión`,productosComplementarios:"Productos relacionados y de apoyo"})}return c.log(`✅ Extracción completada: ${t.length} productos (CONFIGURACIÓN APLICADA)`),t.slice(0,a.productCount)},extractRandomPrice:()=>{const e=["$47","$67","$97","$127","$197","$297"];return e[Math.floor(Math.random()*e.length)]},extractRandomCommission:()=>{const e=["40%","50%","60%","75%"];return e[Math.floor(Math.random()*e.length)]},extractProductInfoFromResponse:(e,t,o)=>{c.log(`🔍 Extrayendo información real para: ${t}`);const a=e.split(`
`);let n={nombre:t,precio:"",comision:"",score:0,gravity:"",descripcion:"",painPoints:"",emociones:"",triggers:"",cvrEstimado:"",epcEstimado:"",aov:"",cpaEstimado:"",roiReal:"",breakEven:"",profitMargin:"",estacionalidad:"",horarioOptimo:"",competenciaNivel:"",programas:"",estrategia:"",productosComplementarios:""};const i=a.findIndex(l=>l.toLowerCase().includes(t.toLowerCase().substring(0,10)));if(i!==-1){const l=Math.max(0,i-5),d=Math.min(a.length,i+20),u=a.slice(l,d).join(`
`);[{field:"precio",patterns:[/precio[:\s]*\$?(\d+)/i,/\$(\d+)/]},{field:"comision",patterns:[/comisi[óo]n[:\s]*(\d+%)/i,/(\d+%)/]},{field:"score",patterns:[/score[:\s]*(\d+)/i,/puntuaci[óo]n[:\s]*(\d+)/i]},{field:"gravity",patterns:[/gravity[:\s]*(\d+)/i,/popularidad[:\s]*(\d+)/i]},{field:"descripcion",patterns:[/descripci[óo]n[:\s]*([^\n]+)/i,/sobre[:\s]*([^\n]+)/i]},{field:"painPoints",patterns:[/problemas?[:\s]*([^\n]+)/i,/dolor[:\s]*([^\n]+)/i,/necesidades?[:\s]*([^\n]+)/i]},{field:"emociones",patterns:[/emociones?[:\s]*([^\n]+)/i,/sentimientos?[:\s]*([^\n]+)/i]},{field:"triggers",patterns:[/triggers?[:\s]*([^\n]+)/i,/gatillos?[:\s]*([^\n]+)/i]},{field:"programas",patterns:[/programas?[:\s]*([^\n]+)/i,/afiliados?[:\s]*([^\n]+)/i,/plataformas?[:\s]*([^\n]+)/i]},{field:"estrategia",patterns:[/estrategia[:\s]*([^\n]+)/i,/marketing[:\s]*([^\n]+)/i]}].forEach(({field:g,patterns:v})=>{for(const C of v){const T=u.match(C);if(T&&T[1]){g==="score"?n[g]=parseInt(T[1])||Math.floor(Math.random()*30)+70:n[g]=T[1].trim();break}}})}n.precio||(n.precio=h.extractRandomPrice()),n.comision||(n.comision=h.extractRandomCommission()),n.score||(n.score=Math.floor(Math.random()*30)+70);const r=document.getElementById("nicho")?.value||"marketing",s=document.getElementById("publico")?.value||"audiencia";return n.descripcion||(n.descripcion=`Producto especializado en ${r} con enfoque en resultados prácticos y aplicables para ${s}.`),n.painPoints||(n.painPoints=`Falta de conocimiento especializado en ${r}, dificultad para obtener resultados consistentes, necesidad de estrategias probadas.`),n.emociones||(n.emociones="Frustración por falta de resultados, deseo de éxito, aspiración al crecimiento profesional"),n.triggers||(n.triggers="Urgencia por resultados, escasez de tiempo, autoridad del experto, prueba social"),n.programas||(n.programas="ClickBank, ShareASale, Commission Junction - Programas confiables con buenas comisiones"),n.estrategia||(n.estrategia=`Estrategia de marketing para ${r}: contenido educativo, testimonios reales, garantías sólidas y enfoque en la transformación del cliente.`),n.productosComplementarios||(n.productosComplementarios=`Herramientas adicionales para ${r}, recursos de apoyo, comunidad premium y actualizaciones continuas.`),c.log(`✅ Información extraída para ${t}:`,n),n},generateAdditionalProducts:e=>{const t=[],o=document.getElementById("nicho").value||"marketing",a=document.getElementById("publico").value||"audiencia",n=[`Curso Avanzado de ${o}`,`Masterclass Completa de ${o}`,`Sistema Premium de ${o}`,`Guía Definitiva de ${o}`,`Entrenamiento VIP de ${o}`,`Blueprint de ${o}`,`Manual Profesional de ${o}`,`Estrategias Avanzadas de ${o}`],i=3-e;for(let r=0;r<i;r++){const s=n[Math.floor(Math.random()*n.length)];t.push({nombre:s,precio:h.extractRandomPrice(),comision:h.extractRandomCommission(),score:Math.floor(Math.random()*25)+70,gravity:Math.floor(Math.random()*50)+20,descripcion:`Producto especializado en ${o} dirigido a ${a}. Ofrece contenido avanzado y estrategias probadas para obtener resultados específicos en el nicho.`,painPoints:`Falta de conocimiento especializado en ${o}, dificultad para implementar estrategias efectivas, necesidad de resultados rápidos y medibles.`,emociones:"Frustración por falta de resultados, deseo de dominar el nicho, aspiración al éxito profesional",triggers:"Urgencia por resultados, escasez de tiempo, autoridad del experto, prueba social",cvrEstimado:`${(Math.random()*2+1).toFixed(1)}%`,epcEstimado:`$${(Math.random()*2+.5).toFixed(2)}`,aov:`$${Math.floor(Math.random()*50)+50}`,cpaEstimado:`$${Math.floor(Math.random()*30)+15}`,roiReal:`${Math.floor(Math.random()*3)+2}x`,breakEven:`${Math.floor(Math.random()*15)+7} días`,profitMargin:`${Math.floor(Math.random()*20)+25}%`,estacionalidad:"Todo el año con picos en enero y septiembre",horarioOptimo:"18:00-22:00 horario local",competenciaNivel:["BAJO","MEDIO","ALTO"][Math.floor(Math.random()*3)],programas:"ClickBank, ShareASale, Commission Junction",estrategia:`Estrategia optimizada para ${o}: enfoque en contenido educativo, testimonios reales, garantía de resultados. Ideal para ${a} que buscan soluciones específicas y probadas.`,productosComplementarios:`Herramientas complementarias de ${o}, recursos adicionales, comunidad premium`})}return c.log(`✅ Generados ${i} productos adicionales para completar 3 total`),t},extractAdditionalAnalysis:e=>{const t={nicheAnalysis:"",ecosystemAnalysis:"",veredicto:"BUENO"},o=e.match(/VEREDICTO[^:]*:\s*(\w+)/i);return o&&(t.veredicto=o[1].toUpperCase()),t}},E={displayResults:e=>{const{productos:t,respuestaCompleta:o,nicheAnalysis:a,ecosystemAnalysis:n,veredicto:i}=e;if(document.getElementById("loading").classList.add("hidden"),c.log("Mostrando resultados...",{productosCount:t.length}),t.length===0){c.showStatus("No se pudieron extraer productos válidos. Revisa el debug para más información.","warning"),E.showDebugSection(),o&&(document.getElementById("debugResponse").textContent=o);return}const r=E.calculateMetrics(t,i);E.displayMetrics(r),E.displayProducts(t),(a||n)&&E.displayAdditionalInsights(a,n),document.getElementById("resultados").classList.remove("hidden"),c.showStatus(`✅ ${t.length} productos analizados exitosamente`,"success")},calculateMetrics:(e,t)=>{const o=e.length>0?Math.round(e.reduce((i,r)=>i+(r.score||0),0)/e.length):0,a=e.filter(i=>(i.score||0)>=80).length,n=e.filter(i=>i.tendencia&&(i.tendencia.includes("📈")||i.tendencia.toLowerCase().includes("subiendo")||i.tendencia.toLowerCase().includes("creciendo"))).length;return{scorePromedio:o,totalProductos:e.length,productosAltoScore:a,conTendenciaPositiva:n,veredicto:t||"BUENO"}},displayMetrics:e=>{const o={EXCELENTE:"#48bb78",BUENO:"#68d391",SATURADO:"#f6ad55",EVITAR:"#fc8181"}[e.veredicto]||"#68d391",a=document.getElementById("metricsOverview");a&&(a.innerHTML=`
            ...
            `,a.classList.remove("hidden"))},displayProducts:e=>{const t=document.getElementById("listaProductos");if(!t){c.log("Elemento listaProductos no encontrado",null,"error");return}t.innerHTML="",e.forEach((o,a)=>{const n=E.createProductCard(o,a+1);t.appendChild(n)}),c.log(`Productos mostrados en UI: ${e.length}`)},createProductCard:(e,t)=>{const o=document.createElement("div");o.className="product-opportunity";const a=e.score||0,n=a>=80?"score-high":a>=60?"score-medium":"score-low";let i=`
            <div class="product-title">
                <div class="product-name">${t}. ${e.nombre||"Producto sin nombre"}</div>
                <div class="product-scores">
                    <span class="opportunity-score ${n}">Score: ${a}/100</span>
        ...
        `,displayAdditionalInsights:(e,t)=>{const o=document.getElementById("additionalInsights");if(o&&(e||t)){let a="";e&&(a+=`
                ...
                `),o.innerHTML=a,o.classList.remove("hidden")}},showLoading:()=>{document.getElementById("loading").classList.remove("hidden"),document.getElementById("resultados").classList.add("hidden"),[{delay:0,step:0},{delay:2e3,step:1},{delay:4e3,step:2},{delay:6e3,step:3}].forEach(({delay:t,step:o})=>{setTimeout(()=>{c.updateLoadingStep(o+1)},t)})},showDebugSection:()=>{m.debugMode||(document.getElementById("debugSection").classList.remove("hidden"),m.debugMode=!0)}},S={copyToClipboard:()=>{if(m.productosDetectados.length===0){c.showStatus("No hay productos para copiar","warning");return}const e=S.generateTextReport();navigator.clipboard.writeText(e).then(()=>{c.showStatus("Análisis copiado al portapapeles","success")}).catch(()=>{c.showStatus("Error al copiar al portapapeles","error")})},downloadText:()=>{if(m.productosDetectados.length===0){c.showStatus("No hay productos para descargar","warning");return}const e=S.generateTextReport();S.downloadFile(e,"analisis-productos-ganadores.txt","text/plain"),c.showStatus("Archivo TXT descargado","success")},exportCSV:()=>{if(m.productosDetectados.length===0){c.showStatus("No hay productos para exportar","warning");return}const e=S.generateCSVReport();S.downloadFile(e,"productos-afiliados.csv","text/csv"),c.showStatus("CSV exportado exitosamente","success")},generateTextReport:()=>{let e=`💰 ANÁLISIS EXPERTO DE PRODUCTOS GANADORES
...
OBJETIVO: Contenido que genere engagement masivo Y conversiones reales.`},processContentResponse:e=>{c.log("Procesando respuesta de contenido...",{length:e.length});const t={};return f.selectedTypes.forEach(o=>{const a=new RegExp(`=== TIPO: ${o.toUpperCase()} ===([\\s\\S]*?)(?==== FIN TIPO|=== TIPO:|$)`,"i"),n=e.match(a);if(n)t[o]=f.parseContentByType(o,n[1]),c.log(`Contenido extraído para ${o}`,t[o]);else{const i=new RegExp(`\\*\\*${o.toUpperCase()}[^:]*:\\*\\*([\\s\\S]*?)(?=\\*\\*[A-Z]+|$)`,"i"),r=e.match(i);r&&(t[o]=f.parseContentByType(o,r[1]),c.log(`Contenido extraído (alternativo) para ${o}`,t[o]))}}),{contenidoPorTipo:t,respuestaCompleta:e}},parseContentByType:(e,t)=>{const o={tipo:e,items:[]},a=(i,r)=>{const s=new RegExp(`${i}:\\s*([^\\n]+)`,"i"),l=r.match(s);return l?l[1].trim():""},n=(i,r)=>{const s=new RegExp(`${i}:\\s*([\\s\\S]*?)(?=[A-Z_]+:|$)`,"i"),l=r.match(s);return l?l[1].trim():""};switch(e){case"tiktok":o.items.push({nombre:"Script TikTok/Reels",hook:a("HOOK \\(3 seg\\)",t),agitacion:a("AGITACIÓN",t),revelacion:a("REVELACIÓN",t),cta:a("CTA",t),hashtags:a("HASHTAGS",t),musica:a("MÚSICA_SUGERIDA",t),score:a("VIRAL_SCORE",t)});break;case"email":o.items.push({nombre:"Email Marketing",subject1:a("SUBJECT_1",t),subject2:a("SUBJECT_2",t),subject3:a("SUBJECT_3",t),preview:a("PREVIEW",t),apertura:a("APERTURA",t),cuerpo:n("CUERPO",t),ctaButton:a("CTA_BUTTON",t),ps:a("PS",t),openRate:a("OPEN_RATE_ESTIMADO",t),clickRate:a("CLICK_RATE_ESTIMADO",t)});break;case"facebook":o.items.push({nombre:"Facebook Ads",headline1:a("HEADLINE_1",t),headline2:a("HEADLINE_2",t),primaryText:n("PRIMARY_TEXT",t),description:a("DESCRIPTION",t),ctaButton:a("CTA_BUTTON",t),audience:a("AUDIENCE_INSIGHT",t),budget:a("BUDGET_SUGERIDO",t),ctr:a("CTR_ESTIMADO",t),cpc:a("CPC_ESTIMADO",t)});break;case"instagram":o.items.push({nombre:"Instagram Post",captionInicio:a("CAPTION_INICIO",t),captionCompleta:n("CAPTION_COMPLETA",t),hashtagsPrimarios:a("HASHTAGS_PRIMARIOS",t),hashtagsNicho:a("HASHTAGS_NICHO",t),storiesIdeas:a("STORIES_IDEAS",t),engagementRate:a("ENGAGEMENT_RATE_ESTIMADO",t),bestTime:a("BEST_TIME_POST",t)});break;case"blog":o.items.push({nombre:"Artículo de Blog",tituloSeo:a("TITULO_SEO",t),metaDescripcion:a("META_DESCRIPCION",t),h2Principales:a("H2_PRINCIPALES",t),introduccion:n("INTRODUCCION",t),keywordsPrimarias:a("KEYWORDS_PRINCIPALES",t),keywordsLsi:a("KEYWORDS_LSI",t),wordCount:a("WORD_COUNT_SUGERIDO",t),dificultadSeo:a("DIFICULTAD_SEO",t)});break;case"youtube":o.items.push({nombre:"Video de YouTube",titulo1:a("TITULO_1",t),titulo2:a("TITULO_2",t),titulo3:a("TITULO_3",t),thumbnailDesc:a("THUMBNAIL_DESCRIPTION",t),scriptIntro:n("SCRIPT_INTRO",t),ganchos:a("GANCHOS_VIDEO",t),descripcion:n("DESCRIPCION",t),tags:a("TAGS",t),ctrEstimado:a("CTR_ESTIMADO",t),retentionEstimado:a("RETENTION_ESTIMADO",t)});break}return o},displayContent:e=>{const{contenidoPorTipo:t}=e,o=document.getElementById("contentTabs"),a=document.getElementById("contentDisplay");o.innerHTML="",a.innerHTML="",Object.keys(t).forEach((i,r)=>{const s=document.createElement("div");s.className=`content-tab ${r===0?"active":""}`,s.dataset.type=i;const l=f.getTypeIcon(i);s.innerHTML=`${l} ${f.getTypeName(i)}`,s.addEventListener("click",()=>{document.querySelectorAll(".content-tab").forEach(d=>d.classList.remove("active")),s.classList.add("active"),f.showContentForType(i,t[i])}),o.appendChild(s)});const n=Object.keys(t)[0];n&&f.showContentForType(n,t[n]),document.getElementById("contentResults").classList.remove("hidden"),document.getElementById("contentResults").scrollIntoView({behavior:"smooth"})},showContentForType:(e,t)=>{const o=document.getElementById("contentDisplay");if(o.innerHTML="",!t||!t.items||t.items.length===0){o.innerHTML=`
                <div class="loading-content">
                    <p>No se pudo generar contenido para ${f.getTypeName(e)}</p>
                </div>
            `;return}t.items.forEach(a=>{const n=f.createContentItemElement(e,a);o.appendChild(n)})},createContentItemElement:(e,t)=>{const o=document.createElement("div");o.className="content-item";let a=`
            <div class="content-item-header">
                <div class="content-title">${t.nombre}</div>
                ${t.score?`<div class="content-score">Score: ${t.score}</div>`:""}
            </div>
        `;switch(e){case"tiktok":a+=`
                ...
                `;break}return o.innerHTML=a,o},getTypeIcon:e=>({tiktok:"📱",email:"📧",facebook:"📊",instagram:"📸",blog:"✍️",youtube:"🎥"})[e]||"📄",getTypeName:e=>({tiktok:"TikTok/Reels",email:"Email Marketing",facebook:"Facebook Ads",instagram:"Instagram",blog:"Blog/SEO",youtube:"YouTube"})[e]||e},M={generateAvatar:async()=>{if(!m.apiKey){c.showStatus("Configura tu API Key primero","error");return}const e=M.gatherAvatarConfig(),t=document.getElementById("generateAvatarBtn"),o=t.innerHTML;t.innerHTML='<span class="btn-icon">🔄</span><span class="btn-text">Creando Avatar...</span>',t.disabled=!0;try{c.log("Iniciando generación de avatar...",e);const a=M.buildAvatarPrompt(e),n=await A.callGemini(a),i=M.processAvatarResponse(n);M.displayAvatar(i),c.showStatus("✅ Avatar ultra-específico creado exitosamente","success")}catch(a){c.showStatus(`Error creando avatar: ${a.message}`,"error"),c.log("Error en generación de avatar",a,"error")}finally{t.innerHTML=o,t.disabled=!1}},gatherAvatarConfig:()=>{const e=document.getElementById("nicho").value.trim(),t=document.getElementById("publico").value.trim();return{nicho:e,publico:t,gender:document.getElementById("avatarGender").value,age:document.getElementById("avatarAge").value,income:document.getElementById("avatarIncome").value,family:document.getElementById("avatarFamily").value,mainProblem:document.getElementById("avatarMainProblem").value.trim(),mainDesire:document.getElementById("avatarMainDesire").value.trim(),tipoProducto:document.getElementById("tipoProducto").value,canalPrincipal:document.getElementById("canalPrincipal").value}},buildAvatarPrompt:e=>{const{nicho:t,publico:o,gender:a,age:n,income:i,family:r,mainProblem:s,mainDesire:l}=e;return`Actúa como PSICÓLOGO EXPERTO EN MARKETING con doctorado en comportamiento del consumidor y 15+ años analizando audiencias de ${t}.
...
✅ Basado en psicología real del ${n} ${a}
...
OBJETIVO: Avatar tan preciso que cualquier marketer puede hablarle directamente a esta persona y convertir al 3-5x más que con audiencias genéricas.`},processAvatarResponse:e=>{c.log("Procesando respuesta de avatar...",{length:e.length});const t={};return[{section:"perfilDemografico",regex:/PERFIL_DEMOGRAFICO:([\s\S]*?)(?=PSICOGRAFIA_PROFUNDA:|$)/i},{section:"psicografia",regex:/PSICOGRAFIA_PROFUNDA:([\s\S]*?)(?=PAIN_POINTS_ESPECIFICOS:|$)/i},{section:"painPoints",regex:/PAIN_POINTS_ESPECIFICOS:([\s\S]*?)(?=TRIGGERS_EMOCIONALES:|$)/i},{section:"triggers",regex:/TRIGGERS_EMOCIONALES:([\s\S]*?)(?=COMPORTAMIENTO_DIGITAL:|$)/i},{section:"comportamientoDigital",regex:/COMPORTAMIENTO_DIGITAL:([\s\S]*?)(?=OBJECIONES_COMPRA:|$)/i},{section:"objeciones",regex:/OBJECIONES_COMPRA:([\s\S]*?)(?=MOMENTO_COMPRA_IDEAL:|$)/i},{section:"momentoCompra",regex:/MOMENTO_COMPRA_IDEAL:([\s\S]*?)(?=LENGUAJE_TRIBAL:|$)/i},{section:"lenguajeTribal",regex:/LENGUAJE_TRIBAL:([\s\S]*?)(?=PATRON_COMUNICACION:|$)/i},{section:"patronComunicacion",regex:/PATRON_COMUNICACION:([\s\S]*?)(?=ENTORNO_SOCIAL:|$)/i},{section:"entornoSocial",regex:/ENTORNO_SOCIAL:([\s\S]*?)(?=RUTINA_DIARIA:|$)/i},{section:"rutinaDiaria",regex:/RUTINA_DIARIA:([\s\S]*?)(?=GATILLOS_ACCION:|$)/i},{section:"gatillosAccion",regex:/GATILLOS_ACCION:([\s\S]*?)(?==== FIN AVATAR|$)/i}].forEach(({section:a,regex:n})=>{const i=e.match(n);i&&(t[a]=i[1].trim())}),{avatar:t,respuestaCompleta:e}},displayAvatar:e=>{const{avatar:t}=e,o=document.getElementById("avatarDisplay");o.innerHTML="",[{key:"perfilDemografico",title:"👤 Perfil Demográfico",icon:"👤"},{key:"psicografia",title:"🧠 Psicografía Profunda",icon:"🧠"},{key:"painPoints",title:"😰 Pain Points Específicos",icon:"😰"},{key:"triggers",title:"🎯 Triggers Emocionales",icon:"🎯"},{key:"comportamientoDigital",title:"📱 Comportamiento Digital",icon:"📱"},{key:"objeciones",title:"🚫 Objeciones de Compra",icon:"🚫"},{key:"momentoCompra",title:"⏰ Momento de Compra Ideal",icon:"⏰"},{key:"lenguajeTribal",title:"💬 Lenguaje Tribal",icon:"💬"},{key:"patronComunicacion",title:"📢 Patrón de Comunicación",icon:"📢"},{key:"entornoSocial",title:"👥 Entorno Social",icon:"👥"},{key:"rutinaDiaria",title:"⏰ Rutina Diaria",icon:"⏰"},{key:"gatillosAccion",title:"🚀 Gatillos de Acción",icon:"🚀"}].forEach(n=>{if(t[n.key]){const i=document.createElement("div");i.className="avatar-section-item",i.innerHTML=`
                ...
                `,o.appendChild(i)}}),document.getElementById("avatarResults").classList.remove("hidden"),document.getElementById("avatarResults").scrollIntoView({behavior:"smooth"})}},R={copyContent:()=>{const e=document.getElementById("contentDisplay");if(!e||e.innerHTML===""){c.showStatus("No hay contenido para copiar","warning");return}const t=R.generateContentReport();navigator.clipboard.writeText(t).then(()=>{c.showStatus("Contenido copiado al portapapeles","success")}).catch(()=>{c.showStatus("Error al copiar contenido","error")})},copyAvatar:()=>{const e=document.getElementById("avatarDisplay");if(!e||e.innerHTML===""){c.showStatus("No hay avatar para copiar","warning");return}const t=R.generateAvatarReport();navigator.clipboard.writeText(t).then(()=>{c.showStatus("Avatar copiado al portapapeles","success")}).catch(()=>{c.showStatus("Error al copiar avatar","error")})},generateContentReport:()=>{let e=`🎯 CONTENIDO VIRAL GENERADO
`;return e+=`🧠 MarketInsight Pro - Generador de Contenido
`,e+=`📅 Fecha: ${new Date().toLocaleDateString()}

`,document.querySelectorAll(".content-tab").forEach(o=>{const a=o.dataset.type,n=f.getTypeName(a);e+=`
=== ${n.toUpperCase()} ===
...
`,document.querySelectorAll(".avatar-section-item").forEach(o=>{const a=o.querySelector(".avatar-section-title").textContent,n=o.querySelector(".avatar-section-content").textContent;e+=`
${a}
`,e+=n+`

`}),e},downloadContent:()=>{const e=R.generateContentReport();S.downloadFile(e,"contenido-viral-generado.txt","text/plain"),c.showStatus("Contenido descargado","success")},downloadAvatar:()=>{const e=R.generateAvatarReport();S.downloadFile(e,"avatar-ultra-especifico.txt","text/plain"),c.showStatus("Avatar descargado","success")}},ne=O.init;O.init=()=>{ne(),f.initTypeSelector(),document.getElementById("generateContentBtn").addEventListener("click",f.generateContent),document.getElementById("copyContentBtn").addEventListener("click",R.copyContent),document.getElementById("downloadContentBtn").addEventListener("click",R.downloadContent),document.getElementById("generateAvatarBtn").addEventListener("click",M.generateAvatar),document.getElementById("copyAvatarBtn").addEventListener("click",R.copyAvatar),document.getElementById("downloadAvatarBtn").addEventListener("click",R.downloadAvatar),c.log("Funcionalidades de Fase 1 inicializadas: Contenido Viral + Avatar Ultra-Específico")};let B=new Set;function ie(){console.log("Inicializando cards de contenido...");const e=document.querySelectorAll(".content-type-card");if(e.length===0){console.log("No se encontraron cards de contenido");return}console.log(`Encontradas ${e.length} cards de contenido`),e.forEach((t,o)=>{console.log(`Configurando card ${o+1}:`,t.dataset.type),t.addEventListener("click",function(a){console.log("Click en card:",this.dataset.type);const n=this.dataset.type;this.classList.contains("selected")?(this.classList.remove("selected"),B.delete(n),console.log(`${n} deseleccionado`)):(this.classList.add("selected"),B.add(n),console.log(`${n} seleccionado`)),console.log("Tipos seleccionados:",Array.from(B))}),t.style.cursor="pointer",t.style.transition="all 0.3s ease"})}const re={integrarConProductos:function(){console.log("🔗 Integrando con productos detectados...");const e=m.productosDetectados||[];if(e.length===0)return console.log("⚠️ No hay productos detectados, usando datos base"),this.generarContextoBase();const t=e[0];return{nombre:t.nombre||"Producto",precio:t.precio||"$97",comision:t.comision||"40%",descripcion:t.descripcion||"",painPoints:this.extraerPainPoints(t),emociones:this.extraerEmociones(t),triggers:this.extraerTriggers(t),nicho:t.nicho||document.getElementById("nicho")?.value||""}},extraerPainPoints:function(e){const t=[];if(e.painPoints&&t.push(...e.painPoints.split(",").map(o=>o.trim())),t.length===0){const o=e.nicho?.toLowerCase()||"";o.includes("peso")||o.includes("fitness")?t.push("No lograr bajar de peso","Falta de energía","No tener tiempo para ejercicio"):o.includes("dinero")||o.includes("financiero")?t.push("Falta de dinero extra","Miedo a las inversiones","No saber por dónde empezar"):t.push("Falta de resultados","Pérdida de tiempo","Frustración constante")}return t.slice(0,3)},extraerEmociones:function(e){const t=[];e.emociones&&t.push(...e.emociones.split(",").map(a=>a.trim()));const o=e.nicho?.toLowerCase()||"";return o.includes("salud")||o.includes("fitness")?t.push("inseguridad","esperanza","determinación"):o.includes("dinero")||o.includes("riqueza")?t.push("ansiedad financiera","ambición","miedo al fracaso"):t.push("frustración","esperanza","urgencia"),[...new Set(t)].slice(0,3)},extraerTriggers:function(e){const t=[];e.triggers&&t.push(...e.triggers.split(",").map(a=>a.trim()));const o=parseFloat(e.precio?.replace(/[^0-9.]/g,"")||"0");return o<50?t.push("precio accesible","riesgo bajo","prueba ahora"):o>200?t.push("inversión seria","exclusividad","resultados premium"):t.push("relación precio-valor","oportunidad","acción inmediata"),t.slice(0,3)},generarContextoBase:function(){const e=document.getElementById("nicho")?.value||"tu nicho";return document.getElementById("publico")?.value,{nombre:"Tu Producto",precio:"$97",comision:"40%",descripcion:`Producto especializado en ${e}`,painPoints:["Falta de resultados","Pérdida de tiempo","Frustración constante"],emociones:["frustración","esperanza","urgencia"],triggers:["oportunidad","cambio","acción inmediata"],nicho:e}}};async function se(){if(console.log("🚀 Generando contenido viral MEJORADO..."),B.size===0){alert("⚠️ Selecciona al menos un tipo de contenido");return}if(!m.apiKey){alert("⚠️ Configura tu API Key primero");return}const e=document.getElementById("generateContentBtn"),t=e.innerHTML;e.innerHTML="🤖 Generando contenido inteligente...",e.disabled=!0;try{const o=re.integrarConProductos();console.log("✅ Contexto del producto:",o);const a={salesAngle:document.getElementById("salesAngle")?.value||"problema-agitacion",controversyLevel:document.getElementById("controversyLevel")?.value||"medium",powerWords:document.getElementById("powerWords")?.value||"gratis, secreto, exclusivo, limitado"},n=Array.from(B),i=o.painPoints[0]||"este problema",r=o.emociones[0]||"frustración",s=o.triggers[0]||"urgencia",l=`Actúa como EXPERTO COPYWRITER VIRAL especializado en marketing de afiliados con +15 años generando $10M+ en ventas.
...
📋 CONFIGURACIÓN:
- Ángulo de venta: ${a.salesAngle}
- Nivel controversia: ${a.controversyLevel}
- Palabras poder: ${a.powerWords}
...
- Lenguaje que convierte en ${o.nicho}`,d=await A.callGemini(l);console.log("🎯 Generando avatar específico para el producto...");let u=null;try{window.AvatarSyncSystem?(u=await AvatarSyncSystem.generarAvatarEspecifico(o,n),console.log("✅ Avatar específico generado:",u.nombre)):console.log("⚠️ AvatarSyncSystem no disponible, continuando sin avatar específico")}catch(p){console.error("Error generando avatar específico:",p)}le(d,n,o,u),c.showStatus(`✅ Contenido inteligente ${u?"+ avatar específico":""} generado para ${n.length} tipos`,"success")}catch(o){console.error("Error:",o),c.showStatus(`❌ Error: ${o.message}`,"error")}finally{e.innerHTML=t,e.disabled=!1}}async function ce(){if(console.log("Generando avatar..."),!m.apiKey){alert("⚠️ Configura tu API Key primero");return}const e=document.getElementById("nicho").value.trim(),t=document.getElementById("publico").value.trim();if(!e||!t){alert("⚠️ Completa el nicho y público objetivo");return}const o=document.getElementById("generateAvatarBtn"),a=o.innerHTML;o.innerHTML="🔄 Creando...",o.disabled=!0;try{const n=`Actúa como PSICÓLOGO EXPERTO EN MARKETING con doctorado en comportamiento del consumidor.
...
Haz este avatar TAN específico que cualquier marketer pueda hablarle directamente y convertir 3-5x más.`,i=await A.callGemini(n);pe(i),c.showStatus("✅ Avatar creado exitosamente","success")}catch(n){console.error("Error:",n),c.showStatus(`❌ Error: ${n.message}`,"error")}finally{o.innerHTML=a,o.disabled=!1}}function le(e,t,o,a=null){let n=document.getElementById("contentResults");if(n||(n=document.createElement("div"),n.id="contentResults",n.className="content-results",document.querySelector(".main-content").appendChild(n)),n.innerHTML=`
        ...
        <div class="content-context" style="background: rgba(0,255,127,0.1); padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #00ff7f;">
            <h3>📊 Contexto del Producto Integrado ${a?"+ Avatar Específico":""}</h3>
            <div class="context-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-top: 10px;">
                ...
                ${a?`
                    <div><strong>👤 Avatar:</strong> ${a.nombre}</div>
                    <div><strong>🎯 Target:</strong> ${a.edad}, ${a.ocupacion}</div>
                `:""}
            </div>
            ${a?`
                <div style="margin-top: 15px; padding: 10px; background: rgba(139, 92, 246, 0.1); border-radius: 6px; border-left: 3px solid #8b5cf6;">
                    <strong>🧠 Avatar Específico Generado:</strong> "${a.nombre}" - ${a.problemaPrincipal} (${a.emocionDominante})
                </div>
            `:""}
        </div>
        ...
        <div class="content-display">
            <div class="content-item">
                <div class="content-title" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                    📱 Contenido Generado para: ${t.map(i=>de(i)+" "+i.toUpperCase()).join(", ")}
                </div>
    ...
    `,n.classList.remove("hidden"),n.scrollIntoView({behavior:"smooth"}),window.lastContentGeneratedEnhanced={respuesta:e,tipos:t,contextoProducto:o,avatarEspecifico:a,timestamp:new Date().toISOString()},a&&window.AvatarSyncSystem)try{AvatarSyncSystem.exportarConjuntoCoherente({respuesta:e,tipos:t,timestamp:new Date().toISOString()},a,o),console.log("✅ Conjunto coherente exportado automáticamente")}catch(i){console.error("Error exportando conjunto coherente:",i)}window.lastContentGenerated=e}function de(e){return{tiktok:"📱",instagram:"📸",facebook:"📊",email:"📧",youtube:"🎥",blog:"✍️"}[e]||"📄"}function pe(e){let t=document.getElementById("avatarResults");t||(t=document.createElement("div"),t.id="avatarResults",t.className="avatar-results",document.querySelector(".main-content").appendChild(t)),t.innerHTML=`
    ...
    `,t.classList.remove("hidden"),t.scrollIntoView({behavior:"smooth"}),window.lastAvatarGenerated=e}function k(){console.log("Inicializando nuevas funcionalidades..."),setTimeout(ie,100);const e=document.getElementById("generateContentBtn"),t=document.getElementById("generateAvatarBtn");e&&(e.onclick=se,console.log("Botón contenido configurado")),t&&(t.onclick=ce,console.log("Botón avatar configurado")),console.log("Nuevas funcionalidades inicializadas correctamente")}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",k):k();setTimeout(k,500);setTimeout(k,1500);async function me(){if(console.log("Generando múltiples avatares automáticamente..."),!m.apiKey){alert("⚠️ Configura tu API Key primero");return}const e={nicho:document.getElementById("nicho").value.trim(),publico:document.getElementById("publico").value.trim(),tipoProducto:document.getElementById("tipoProducto").value,rangoPrecios:document.getElementById("rangoPrecios").value,canalPrincipal:document.getElementById("canalPrincipal").value,presupuestoAds:document.getElementById("presupuestoAds").value,roiObjetivo:document.getElementById("roiObjetivo").value,mercadoGeo:document.getElementById("mercadoGeo").value,dispositivoTarget:document.getElementById("dispositivoTarget").value,experiencia:document.getElementById("experiencia").value};if(!e.nicho||!e.publico){alert("⚠️ Completa el nicho y público objetivo primero");return}const t=document.getElementById("generateMultipleAvatarsBtn"),o=t.innerHTML;t.innerHTML="🤖 Generando 5 Avatares...",t.disabled=!0;try{const a=ue(e),n=await A.callGemini(a);ge(n),c.showStatus("✅ 5 avatares generados automáticamente","success")}catch(a){console.error("Error:",a),c.showStatus(`❌ Error: ${a.message}`,"error")}finally{t.innerHTML=o,t.disabled=!1}}function ue(e){const{nicho:t,publico:o,tipoProducto:a,rangoPrecios:n,canalPrincipal:i,presupuestoAds:r,roiObjetivo:s,mercadoGeo:l,dispositivoTarget:d}=e;return`Actúa como EXPERTO EN SEGMENTACIÓN DE AUDIENCIAS con 15+ años creando avatares ultra-específicos.
...
OBJETIVO: 5 avatares TAN específicos que puedas crear campañas ultra-dirigidas para cada uno con mensajes completamente diferentes.`}function ge(e){let t=document.getElementById("multipleAvatarsResults");t||(t=document.createElement("div"),t.id="multipleAvatarsResults",t.className="multiple-avatars-results",document.querySelector(".main-content").appendChild(t));const o=Ee(e);let a=`
    ...
    `;o.forEach((n,i)=>{a+=`
            <div class="avatar-card">
                ...
                <div class="avatar-content">
                    <pre style="white-space: pre-wrap; font-family: inherit; line-height: 1.5; font-size: 0.9rem;">${n.contenido}</pre>
                </div>
    ...
    `,t.innerHTML=a,t.classList.remove("hidden"),t.scrollIntoView({behavior:"smooth"}),window.lastMultipleAvatars=e,window.processedAvatars=o}function Ee(e){const t=[],o=/=== AVATAR \d+: ([^=]+) ===([\s\S]*?)(?==== AVATAR \d+:|$)/g;let a;for(;(a=o.exec(e))!==null;)t.push({titulo:a[1].trim(),contenido:a[2].trim()});return t.length===0&&e.split(/AVATAR \d+:/).forEach((i,r)=>{i.trim()&&r>0&&t.push({titulo:`Avatar ${r}`,contenido:i.trim()})}),t}function ve(){const e=document.getElementById("generateAvatarBtn");if(e&&!document.getElementById("generateMultipleAvatarsBtn")){const t=document.createElement("button");t.className="btn btn-avatar",t.id="generateMultipleAvatarsBtn",t.style.marginTop="15px",t.innerHTML='<span class="btn-icon">🤖</span><span class="btn-text">Generar 5 Avatares Automáticamente</span>',t.onclick=me,e.parentNode.appendChild(t)}}const fe=`
...
`;function Ie(){if(!document.getElementById("multipleAvatarsCSS")){const e=document.createElement("style");e.id="multipleAvatarsCSS",e.textContent=fe,document.head.appendChild(e)}}function V(){Ie(),setTimeout(ve,1e3),console.log("Auto-generador de avatares múltiples inicializado")}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",V):V();setTimeout(V,1e3);const z=window.mostrarResultadosAvatar||function(){};E.displayResults;window.mostrarResultadosAvatar=function(e){window.lastAvatarGenerated=e,typeof z=="function"&&z(e),setTimeout(updateFunnelExportButton,500),console.log("✅ Avatar guardado globalmente")};if(typeof E<"u"&&E.displayResults){const e=E.displayResults;E.displayResults=function(t){t&&t.productos&&(typeof m>"u"&&(window.AppState={}),m.productosDetectados=t.productos,console.log("✅ Productos guardados globalmente:",t.productos.length)),e.call(this,t),setTimeout(updateFunnelExportButton,500)}}setInterval(function(){const e=!!window.lastAvatarGenerated,t=!!(typeof m<"u"&&m.productosDetectados&&m.productosDetectados.length>0);(e||t)&&updateFunnelExportButton&&updateFunnelExportButton()},2e3);document.addEventListener("DOMContentLoaded",function(){setTimeout(function(){const e=document.getElementById("avatarResults"),t=document.getElementById("resultados");if(e&&!e.classList.contains("hidden")){console.log("🔍 Avatar detectado en pantalla, buscando datos...");const o=e.querySelector(".avatar-content, .avatar-display");o&&o.textContent&&(window.lastAvatarGenerated=o.textContent,console.log("✅ Avatar recuperado del DOM"))}t&&!t.classList.contains("hidden")&&(console.log("🔍 Productos detectados en pantalla, simulando datos..."),typeof m>"u"&&(window.AppState={}),m.productosDetectados||(m.productosDetectados=[{nombre:"Producto detectado",precio:"$50-200",descripcion:"Producto de fitness y bienestar"}],console.log("✅ Productos simulados"))),updateFunnelExportButton&&updateFunnelExportButton()},1e3)});console.log("🔧 Fix de variables globales cargado");const P={openTrendPredictor:()=>{console.log("🔮 Abriendo Trend Predictor...");const e={nicho:document.getElementById("nicho")?.value?.trim()||"",mercado:document.getElementById("mercadoGeo")?.value||"LATAM",tipoProducto:document.getElementById("tipoProducto")?.value||"digital",canalPrincipal:document.getElementById("canalPrincipal")?.value||"paid",presupuestoAds:document.getElementById("presupuestoAds")?.value||"1000",experiencia:document.getElementById("experiencia")?.value||"intermedio"};if(!e.nicho){alert("⚠️ Ingresa un nicho primero");return}localStorage.setItem("main_nicho",e.nicho),localStorage.setItem("main_mercado",e.mercado),localStorage.setItem("main_config",JSON.stringify(e));const o=`trend-predictor.html?${new URLSearchParams({nicho:e.nicho,mercado:e.mercado,source:"marketinsight-pro"}).toString()}`;window.open(o,"_blank","width=1400,height=900,scrollbars=yes,resizable=yes")?console.log("✅ Trend Predictor abierto exitosamente"):alert("⚠️ Permitir pop-ups para abrir Trend Predictor"),typeof c<"u"&&c.showStatus&&c.showStatus(`🔮 Trend Predictor abierto para: ${e.nicho}`,"success"),console.log("🔮 Configuración enviada:",e)},canUseTrendPredictor:()=>{const e=localStorage.getItem("gemini_api_key"),t=document.getElementById("nicho")?.value?.trim();return!!(e&&t)},updateTrendButton:()=>{const e=document.getElementById("openTrendPredictorBtn");if(!e)return;P.canUseTrendPredictor();const t=document.getElementById("nicho")?.value?.trim()||"";localStorage.getItem("gemini_api_key")?t?(e.style.opacity="1",e.disabled=!1,e.innerHTML=`🔮 Predecir Tendencias: ${t}`):(e.style.opacity="0.6",e.disabled=!0,e.innerHTML="🔮 Trend Predictor (Ingresa nicho primero)"):(e.style.opacity="0.6",e.disabled=!0,e.innerHTML="🔮 Trend Predictor (Configura API Key primero)")}};function J(){const e=document.getElementById("generateBtn");if(!e){setTimeout(J,1e3);return}if(document.getElementById("openTrendPredictorBtn"))return;console.log("📋 Agregando botón Trend Predictor...");const t=document.createElement("button");t.id="openTrendPredictorBtn",t.className="btn btn-primary",t.style.cssText=`
        ...
        width: 100% !important;
    `,t.innerHTML="🔮 Trend Predictor (Configura nicho primero)",t.onclick=P.openTrendPredictor,e.parentNode.insertBefore(t,e.nextSibling),P.updateTrendButton(),console.log("✅ Botón Trend Predictor agregado exitosamente")}function he(){["nicho","mercadoGeo"].forEach(t=>{const o=document.getElementById(t);o&&(o.addEventListener("input",P.updateTrendButton),o.addEventListener("change",P.updateTrendButton),o.addEventListener("keyup",P.updateTrendButton))}),setInterval(P.updateTrendButton,3e3),console.log("👂 Listeners configurados para Trend Predictor")}function Ae(){if(document.getElementById("trendPredictorStyles"))return;const e=`
    ...
    `,t=document.createElement("style");t.id="trendPredictorStyles",t.textContent=e,document.head.appendChild(t),console.log("🎨 Estilos Trend Predictor agregados")}function w(){console.log("🔮 Inicializando integración Trend Predictor..."),Ae(),setTimeout(J,1e3),setTimeout(he,1500),new URLSearchParams(window.location.search).get("from")==="trend-predictor"&&(console.log("🔄 Usuario regresando desde Trend Predictor"),typeof c<"u"&&c.showStatus&&c.showStatus("🔮 Datos de tendencias disponibles para análisis","info")),console.log("✅ Integración Trend Predictor inicializada completamente")}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",w):w();setTimeout(w,2e3);setTimeout(()=>{document.getElementById("openTrendPredictorBtn")||(console.log("🔄 Ejecutando respaldo de inicialización..."),w())},4e3);function Ce(){console.log("🔧 DEBUG TREND PREDICTOR:"),console.log("- API Key:",!!localStorage.getItem("gemini_api_key")),console.log("- Nicho campo:",document.getElementById("nicho")?.value||"NO ENCONTRADO"),console.log("- Botón existe:",!!document.getElementById("openTrendPredictorBtn")),console.log("- Estilos cargados:",!!document.getElementById("trendPredictorStyles"));const e=document.getElementById("openTrendPredictorBtn");e&&(console.log("- Botón habilitado:",!e.disabled),console.log("- Texto del botón:",e.textContent))}window.debugTrendPredictor=Ce;console.log("🔮 Trend Predictor Integration cargado. Usa debugTrendPredictor() para troubleshooting.");const $={validateOffer:async(e,t)=>{if(!m.apiKey){alert("⚠️ Configura tu API Key primero");return}const o=`Actúa como SUPER AFILIADO EXPERTO con 15+ años en ClickBank, ShareASale, CJ, MaxBounty y acceso a datos internos de networks.
...
CONTEXTO ESPECÍFICO: Analizar para ${t} considerando competencia actual 2025, tendencias de conversión, y comportamiento de audiencia específica.`;try{const a=await A.callGemini(o);return $.parseValidationResponse(a)}catch(a){return console.error("Error validando oferta:",a),null}},parseValidationResponse:e=>{console.log("🔍 Parseando respuesta de validación:",e.substring(0,200)+"...");const t=(n,i="0")=>n&&n.replace(/[^0-9.]/g,"")||i,o=(n,i="")=>n&&n.trim()||i,a={exists:e.match(/EXISTE_EN_NETWORKS:\s*\[?SI\]?/i)!==null||e.includes("disponible")||e.includes("activo")||!e.includes("NO EXISTE"),gravity:(()=>{const n=[/Gravity\s*(?:Score)?:\s*\[?(\d+)\]?/i,/GRAVITY:\s*\[?(\d+)\]?/i,/Popularidad:\s*\[?(\d+)\]?/i,/Score:\s*(\d+)/i];for(const i of n){const r=e.match(i);if(r)return t(r[1],"35")}return e.includes("WINNER")||e.includes("EXCELENTE")?"65":e.includes("PROMETEDOR")||e.includes("BUENO")?"45":e.includes("SATURADO")?"25":e.includes("EVITAR")?"15":"35"})(),epc:(()=>{const n=[/EPC\s*(?:Promedio|Estimado)?:\s*\[\$?([\d.]+)\]/i,/EPC_[A-Z_]*:\s*\$?([\d.]+)/i,/Earnings?\s*per\s*Click:\s*\$?([\d.]+)/i,/\$?([\d.]+)\s*(?:por|per)\s*click/i];for(const i of n){const r=e.match(i);if(r)return t(r[1],"0")}return"0"})(),conversionRate:(()=>{const n=[/Conversion\s*Rate:\s*\[?([\d.]+)%?\]?/i,/CVR[^:]*:\s*\[?([\d.]+)%?\]?/i,/CR:\s*\[?([\d.]+)%?\]?/i,/Conversi[oó]n:\s*([\d.]+)%?/i];for(const i of n){const r=e.match(i);if(r)return t(r[1],"0")}return"0"})(),verdict:(()=>{const n=[/VERDICT:\s*\[?(\w+)\]?/i,/VEREDICTO:\s*\[?(\w+)\]?/i,/Veredicto:\s*(\w+)/i,/Recomendaci[oó]n:\s*(\w+)/i];for(const i of n){const r=e.match(i);if(r)return r[1].toUpperCase()}return e.includes("WINNER")||e.includes("excelente oportunidad")?"WINNER":e.includes("PROMETEDOR")||e.includes("buena opción")?"PROMETEDOR":e.includes("SATURADO")||e.includes("muy competido")?"SATURADO":e.includes("EVITAR")||e.includes("no recomendado")?"EVITAR":"PROMETEDOR"})(),competitionLevel:(()=>{const n=[/Saturaci[oó]n:\s*\[?(\w+)\]?/i,/Competencia:\s*\[?(\w+)\]?/i,/Competition:\s*\[?(\w+)\]?/i];for(const i of n){const r=e.match(i);if(r)return r[1].toUpperCase()}return e.includes("alta competencia")||e.includes("muy saturado")?"ALTA":e.includes("competencia media")||e.includes("moderadamente")?"MEDIA":e.includes("baja competencia")||e.includes("nicho nuevo")?"BAJA":"MEDIA"})(),networks:(()=>{const n=e.match(/NETWORKS_DISPONIBLES:\s*\[([^\]]+)\]/i);if(n)return o(n[1],"");const i=[];return e.includes("ClickBank")&&i.push("ClickBank"),e.includes("ShareASale")&&i.push("ShareASale"),(e.includes("CJ")||e.includes("Commission Junction"))&&i.push("CJ"),e.includes("MaxBounty")&&i.push("MaxBounty"),e.includes("Amazon")&&i.push("Amazon Associates"),i.join(", ")||"ClickBank, ShareASale"})(),profitEstimate:(()=>{const n=[/Profit\s*(?:Est|Estimado)?:\s*\[\$?([\d,]+)\]/i,/Ganancia:\s*\$?([\d,]+)/i,/Revenue\s*Est:\s*\$?([\d,]+)/i];for(const l of n){const d=e.match(l);if(d)return t(d[1],"0")}const i=parseInt(a.gravity)||35,r=parseFloat(a.epc)||1.5;return Math.round(i*r*10).toString()})(),tips:(()=>{const n=[/TIPS_SECRETOS:\s*\n([\s\S]*?)(?==== FIN|VEREDICTO|$)/i,/Tips?[^:]*:\s*\n([\s\S]*?)(?=\n[A-Z_]+:|$)/i,/Recomendaciones:\s*\n([\s\S]*?)(?=\n[A-Z_]+:|$)/i];for(const i of n){const r=e.match(i);if(r)return r[1].split(/\d+\.\s*/).filter(s=>s.trim()).map(s=>s.trim()).join(`
• `).substring(0,500)}return"Tips específicos no disponibles en esta validación."})(),cpaEstimado:(()=>{const n=e.match(/CPA[^:]*:\s*\$?([\d.]+)/i);return n?`$${t(n[1],"15")}`:""})(),roiEstimado:(()=>{const n=e.match(/ROI[^:]*:\s*([\d.]+)x?/i);return n?`${t(n[1],"3")}x`:""})(),refundRate:(()=>{const n=e.match(/Refund\s*Rate:\s*([\d.]+)%?/i);return n?`${t(n[1],"5")}%`:""})(),cookieDuration:(()=>{const n=e.match(/Cookie\s*Duration:\s*([\d]+)\s*d[ií]as?/i);return n?`${n[1]} días`:""})()};return console.log("✅ Datos extraídos de validación:",a),a},displayValidation:(e,t,o)=>{const a=o.querySelector(".offer-validation");a&&a.remove(),console.log("🎯 Mostrando validación completa:",e);const n={WINNER:"winner",PROMETEDOR:"prometedor",SATURADO:"saturado",EVITAR:"evitar",UNKNOWN:"unknown"}[e.verdict]||"prometedor",i=(d,u)=>{const p=parseFloat(d)||0;return p>=u.good?"good":p>=u.medium?"medium":"bad"},r=`
        <div class="offer-validation ${n}">
...
`).filter(d=>d.trim()).map((d,u)=>`
    ...
    `,document.body.appendChild(t),setTimeout(()=>{t.style.opacity="0",setTimeout(()=>t.remove(),300)},3e3)}};function _(){document.querySelectorAll(".product-opportunity").forEach((e,t)=>{if(!e.querySelector(".validate-btn")){let o=e.querySelector(".product-actions");o||(o=document.createElement("div"),o.className="product-actions",e.appendChild(o));const a=document.createElement("button");a.className="btn btn-secondary validate-btn",a.innerHTML="🔍 Validar Oferta",a.dataset.productIndex=t,a.onclick=async function(){const n=m.productosDetectados[this.dataset.productIndex];this.disabled=!0,this.innerHTML="🔄 Validando...";try{const i=await $.validateOffer(n.nombre,document.getElementById("nicho").value);i&&$.displayValidation(i,n.nombre,e)}catch(i){console.error("Error validando:",i),alert("Error al validar. Intenta de nuevo.")}finally{this.disabled=!1,this.innerHTML="🔍 Validar Oferta"}},o.appendChild(a)}})}const L={spiedProducts:new Set,spyWinningAds:async(e,t,o)=>{if(!m.apiKey){alert("⚠️ Configura tu API Key primero");return}const a=`Actúa como EXPERTO EN FACEBOOK ADS LIBRARY y TIKTOK CREATIVE CENTER con acceso completo a todas las campañas activas.
...
=== FIN ANALYSIS ===`;try{const n=await A.callGemini(a);return L.parseSpyResponse(n)}catch(n){return console.error("Error en spy creatives:",n),null}},parseSpyResponse:e=>{const t={hooks:[],angles:[],copyFramework:"",metrics:{},audiences:[],visualElements:"",schedule:""},o=e.match(/Hook #\d+: ([^\n]+)/gi);o&&(t.hooks=o.map(s=>s.replace(/Hook #\d+: /i,"")));const a=e.match(/ÁNGULOS QUE CONVIERTEN:([\s\S]*?)FORMATO DE CREATIVOS/i);a&&(t.angles=a[1].trim());const n=e.match(/COPY FRAMEWORK GANADOR:([\s\S]*?)AD METRICS/i);n&&(t.copyFramework=n[1].trim()),t.metrics={ctr:e.match(/CTR:\s*\[?([\d.]+)\]?%/i)?.[1]||"2.5",cpc:e.match(/CPC:\s*\$\[?([\d.]+)\]/i)?.[1]||"0.75",cpm:e.match(/CPM:\s*\$\[?([\d.]+)\]/i)?.[1]||"15.00",cvr:e.match(/Conversion Rate:\s*\[?([\d.]+)\]?%/i)?.[1]||"2.0",roas:e.match(/ROAS esperado:\s*\[?([\d.]+)\]?x/i)?.[1]||"3.0"};const i=e.match(/\d+\.\s*\[([^\]]+)\]/g);i&&(t.audiences=i.map(s=>s.replace(/\d+\.\s*\[|\]/g,"")));const r=e.match(/(?:AUDIENCIAS GANADORAS|Intereses TOP):([\s\S]*?)(?=ELEMENTOS VISUALES|HORARIOS|$)/i);if(r){const s=r[1],l=[/\d+\.\s*\[([^\]]+)\]/g,/\d+\.\s*([^[\n]+)/g,/- ([^[\n]+)/g,/• ([^[\n]+)/g];for(const d of l){const u=s.matchAll(d);for(const p of u){const g=p[1].trim();g&&!g.includes("[")&&g.length>3&&t.audiences.push(g)}}t.audiences.length===0&&s.split(`
`).forEach(u=>{const p=u.trim().replace(/^[-•*]\s*/,"");p&&p.length>3&&!p.includes(":")&&t.audiences.push(p)})}return t},displaySpyResults:(e,t,o)=>{const a=`
            ...
            `,n=document.createElement("div");n.innerHTML=a,n.className="spy-container",o.appendChild(n),setTimeout(()=>{n.querySelector(".spy-results").classList.add("show")},100)},copyText:e=>{navigator.clipboard.writeText(e).then(()=>{const t=document.createElement("div");t.className="copy-notification",t.textContent="✅ Copiado!",document.body.appendChild(t),setTimeout(()=>{t.remove()},2e3)})},generateVariants:async e=>{alert(`🎨 Función "Generar 10 Variantes" próximamente...

Por ahora, usa los hooks y ángulos proporcionados para crear tus propias variantes.`)},exportAdTemplate:e=>{const t=document.getElementById(`spy-${e.replace(/\s+/g,"-")}`);if(t){const o=t.innerText,a=new Blob([o],{type:"text/plain"}),n=URL.createObjectURL(a),i=document.createElement("a");i.href=n,i.download=`ad-template-${e.replace(/\s+/g,"-")}.txt`,i.click(),URL.revokeObjectURL(n)}}};function D(){document.querySelectorAll(".product-opportunity").forEach((e,t)=>{if(!e.querySelector(".spy-btn")){const o=e.querySelector(".validate-btn")?.parentElement||e,a=document.createElement("button");a.className="btn btn-secondary spy-btn",a.innerHTML="🕵️ Spy Creativos",a.style.marginTop="10px",a.style.marginLeft="10px",a.onclick=async()=>{const n=m.productosDetectados[t];if(L.spiedProducts.has(t)){const r=e.querySelector(".spy-results");r&&(r.style.display=r.style.display==="none"?"block":"none");return}a.disabled=!0,a.innerHTML="🔄 Analizando creativos...";const i=await L.spyWinningAds(n.nombre,document.getElementById("nicho").value,t);i&&(L.displaySpyResults(i,n.nombre,e),L.spiedProducts.add(t)),a.disabled=!1,a.innerHTML="🕵️ Spy Creativos"},e.querySelector(".validate-btn")?e.querySelector(".validate-btn").after(a):o.appendChild(a)}})}const be=_;_=function(){be(),setTimeout(D,100)};const H={currentProduct:null,currentScenarios:null,open:function(e,t){console.log("🧮 Abriendo Profit Calculator para:",e.nombre),this.currentProduct=e,document.getElementById("calcProductName").textContent=e.nombre||"Producto",document.getElementById("calcProductPrice").textContent=e.precio||"$97",document.getElementById("calcProductCommission").textContent=e.comision||"40%",document.getElementById("calculatorResults").classList.add("hidden"),document.getElementById("profitCalculatorModal").classList.remove("hidden")},closeModal:function(){document.getElementById("profitCalculatorModal").classList.add("hidden")},calculate:async function(){if(console.log("🧮 Iniciando cálculo de profit..."),!this.currentProduct){alert("⚠️ No hay producto seleccionado");return}if(!localStorage.getItem("gemini_api_key")){alert("⚠️ API Key no encontrada. Configúrala en MarketInsight Pro primero.");return}const t={budget:parseFloat(document.getElementById("calcBudget").value)||50,channel:document.getElementById("calcChannel").value,days:parseInt(document.getElementById("calcDays").value)||30,market:document.getElementById("calcMarket").value};console.log("⚙️ Configuración:",t);const o=document.querySelector(".btn-calculate"),a=o.innerHTML;o.innerHTML="<span>🔄</span><span>Calculando...</span>",o.disabled=!0;try{const n=this.buildCalculationPrompt(t);console.log("📝 Prompt construido, longitud:",n.length);const i=await this.callGeminiForCalculations(n);console.log("📥 Respuesta recibida de IA");const r=this.parseCalculationResponse(i);console.log("📊 Escenarios procesados:",r),this.currentScenarios=r,this.displayScenarios(r),this.drawScalingChart(r),document.getElementById("calculatorResults").classList.remove("hidden"),console.log("✅ Cálculo completado exitosamente")}catch(n){console.error("❌ Error calculando:",n),alert(`Error al calcular: ${n.message}`)}finally{o.innerHTML=a,o.disabled=!1}},buildCalculationPrompt:function(e){const t=this.currentProduct;let o=97,a=40;if(t.precio&&typeof t.precio=="string"){const r=t.precio.match(/[\d,]+\.?\d*/);r&&r[0]&&(o=parseFloat(r[0].replace(/,/g,"")))}if(t.comision&&typeof t.comision=="string"){const r=t.comision.match(/\d+/);r&&r[0]&&(a=parseInt(r[0]))}const n=(o*a/100).toFixed(2),i=document.getElementById("nicho")?.value||"General";return`Eres un MEDIA BUYER EXPERTO con 10+ años comprando tráfico para productos de afiliados.

PRODUCTO A ANALIZAR:
- Nombre: ${t.nombre}
- Precio: $${o}
- Comisión: ${a}% ($${n} por venta)
- Nicho: ${i}
...
IMPORTANTE: Con $${e.budget}/día es IMPOSIBLE que CPC sea $0 o que no haya clicks. Calcula números REALES.`},getTypicalCPC:function(e,t){return{facebook:{tier1:1.5,tier2:.8,tier3:.4},google:{tier1:2.2,tier2:1.2,tier3:.6},tiktok:{tier1:1.8,tier2:1,tier3:.5},native:{tier1:.9,tier2:.5,tier3:.25}}[e]?.[t]||1},getNicheCompetition:function(e){const t=["fitness","weight loss","make money","crypto","forex"],o=["beauty","health","relationships","self help"];return t.some(a=>e.toLowerCase().includes(a))?"ALTA":o.some(a=>e.toLowerCase().includes(a))?"MEDIA":"BAJA"},getPriceRange:function(e){return e<50?"BAJO":e<200?"MEDIO":"ALTO"},parseCalculationResponse:function(e){console.log("🔄 Parseando respuesta:",e.substring(0,200)+"...");const t={conservative:{},realistic:{},optimistic:{},scaling:{},recommendations:""},o=e.match(/=== ESCENARIO CONSERVADOR ===([\s\S]*?)(?==== ESCENARIO REALISTA|$)/i),a=e.match(/=== ESCENARIO REALISTA ===([\s\S]*?)(?==== ESCENARIO OPTIMISTA|$)/i),n=e.match(/=== ESCENARIO OPTIMISTA ===([\s\S]*?)(?=SCALING PROJECTION|$)/i);o?t.conservative=this.extractMetricsForScenario(o[1],"conservative"):t.conservative=this.generateFallbackScenario("conservative"),a?t.realistic=this.extractMetricsForScenario(a[1],"realistic"):t.realistic=this.generateFallbackScenario("realistic"),n?t.optimistic=this.extractMetricsForScenario(n[1],"optimistic"):t.optimistic=this.generateFallbackScenario("optimistic"),this.ensureDifferentScenarios(t),this.validateCalculationLogic(t);const i=e.match(/SCALING PROJECTION:([\s\S]*?)(?=RECOMENDACIONES|$)/i);if(i){const s=i[1];t.scaling={month1:this.extractNumber(s.match(/Mes_1:\s*\$?([\d,]+)/i)?.[1])||"500",month2:this.extractNumber(s.match(/Mes_2:\s*\$?([\d,]+)/i)?.[1])||"1200",month3:this.extractNumber(s.match(/Mes_3:\s*\$?([\d,]+)/i)?.[1])||"2500"}}else{const s=parseFloat(t.realistic.profit||"0");if(s<0){const l=Math.abs(s);t.scaling={month1:Math.round(s).toString(),month2:Math.round(s*.4).toString(),month3:Math.round(l*.5).toString()}}else t.scaling={month1:Math.round(s).toString(),month2:Math.round(s*1.8).toString(),month3:Math.round(s*2.5).toString()}}const r=e.match(/RECOMENDACIONES[^:]*:([\s\S]*?)$/i);return r&&(t.recommendations=r[1].trim()),console.log("📊 Escenarios finales validados:",t),t},extractMetricsForScenario:function(e,t){return{cpc:this.extractNumber(e.match(/CPC:\s*\$?([\d.]+)/i)?.[1])||"1.50",ctr:this.extractNumber(e.match(/CTR:\s*([\d.]+)%?/i)?.[1])||"2.0",cr:this.extractNumber(e.match(/CR:\s*([\d.]+)%?/i)?.[1])||"1.5",clicks:this.extractNumber(e.match(/Clicks[^:]*:\s*([\d,]+)/i)?.[1])||"1000",conversions:this.extractNumber(e.match(/Conversiones:\s*([\d,]+)/i)?.[1])||"30",revenue:this.extractNumber(e.match(/Revenue:\s*\$?([\d,]+)/i)?.[1])||"1164",adSpend:this.extractNumber(e.match(/Ad_Spend:\s*\$?([\d,]+)/i)?.[1])||(parseFloat(document.getElementById("calcBudget")?.value||"50")*parseInt(document.getElementById("calcDays")?.value||"30")).toString(),profit:this.extractNumber(e.match(/Profit:\s*\$?([\d,.-]+)/i)?.[1])||"0",roi:this.extractNumber(e.match(/ROI:\s*([\d.-]+)%?/i)?.[1])||"0",breakeven:this.extractNumber(e.match(/(?:Dias_breakeven|breakeven):\s*([\d]+)/i)?.[1])||"30"}},validateAndFixScenarios:function(e){console.log("🔍 Validando diferencias entre escenarios...");const t=parseFloat(e.conservative.profit||"0"),o=parseFloat(e.realistic.profit||"0"),a=parseFloat(e.optimistic.profit||"0");if(t===o&&o===a){console.log("⚠️ Escenarios idénticos detectados, regenerando...");const n={budget:parseFloat(document.getElementById("calcBudget").value)||50,days:parseInt(document.getElementById("calcDays").value)||30,channel:document.getElementById("calcChannel").value,market:document.getElementById("calcMarket").value},i=n.budget*n.days,r=38.8;e.conservative={cpc:"2.50",ctr:"1.2",cr:"0.8",clicks:Math.round(i/2.5).toString(),conversions:Math.round(i/2.5*.012*.008).toString(),revenue:Math.round(7*r).toString(),adSpend:i.toString(),profit:(272-i).toString(),roi:"-82",breakeven:"45"},e.realistic={cpc:"1.50",ctr:"2.0",cr:"1.8",clicks:Math.round(i/1.5).toString(),conversions:Math.round(i/1.5*.02*.018).toString(),revenue:Math.round(36*r).toString(),adSpend:i.toString(),profit:(1397-i).toString(),roi:"-7",breakeven:"15"},e.optimistic={cpc:"0.75",ctr:"3.0",cr:"2.5",clicks:Math.round(i/.75).toString(),conversions:Math.round(i/.75*.03*.025).toString(),revenue:Math.round(150*r).toString(),adSpend:i.toString(),profit:(5820-i).toString(),roi:"288",breakeven:"5"},console.log("✅ Escenarios regenerados con valores diferentes")}},generateRealisticFallback:function(e,t){const o=(t.budget||50)*(t.days||30),n={conservative:{cpc:"2.00",ctr:"1.0",cr:"0.5",conversions:8,revenue:310,profit:-1190,roi:-79},realistic:{cpc:"1.20",ctr:"2.0",cr:"1.5",conversions:38,revenue:1474,profit:-26,roi:-2},optimistic:{cpc:"0.60",ctr:"3.5",cr:"3.0",conversions:175,revenue:6790,profit:5290,roi:353}}[e];return{cpc:n.cpc,ctr:n.ctr,cr:n.cr,clicks:Math.round(o/parseFloat(n.cpc)).toString(),conversions:n.conversions.toString(),revenue:n.revenue.toString(),adSpend:o.toString(),profit:n.profit.toString(),roi:n.roi.toString(),breakeven:e==="conservative"?"60":e==="realistic"?"12":"4"}},extractMetricsUnique:function(e,t){console.log(`Extrayendo métricas ${t}:`,e.substring(0,100));const o={cpc:this.extractSingleMetric(e,/CPC:\s*\$?([\d.]+)/i),ctr:this.extractSingleMetric(e,/CTR:\s*([\d.]+)%?/i),cr:this.extractSingleMetric(e,/CR:\s*([\d.]+)%?/i),clicks:this.extractSingleMetric(e,/Clicks[^:]*:\s*([\d,]+)/i),conversions:this.extractSingleMetric(e,/Conversiones:\s*([\d,]+)/i),revenue:this.extractSingleMetric(e,/Revenue:\s*\$?([\d,]+)/i),adSpend:this.extractSingleMetric(e,/Ad_Spend:\s*\$?([\d,]+)/i),profit:this.extractSingleMetric(e,/Profit:\s*\$?([\d,.-]+)/i),roi:this.extractSingleMetric(e,/ROI:\s*([\d.-]+)%?/i),breakeven:this.extractSingleMetric(e,/(?:Dias_breakeven|breakeven):\s*([\d]+)/i)};return!o.cpc||o.cpc==="0"||parseFloat(o.cpc)<=0?this.generateFallbackScenario(t):(console.log(`Métricas ${t} extraídas:`,o),o)},extractSingleMetric:function(e,t){const o=e.match(t);return o&&o[1]?this.extractNumber(o[1]):null},generateFallbackScenario:function(e){console.log(`🛡️ Generando escenario fallback: ${e}`);const t={budget:parseFloat(document.getElementById("calcBudget").value)||50,days:parseInt(document.getElementById("calcDays").value)||30},o=t.budget*t.days,a=38.8,n={conservative:{cpc:2.2,ctr:1.1,cr:.9,expectedROI:-25},realistic:{cpc:1.5,ctr:2,cr:1.7,expectedROI:15},optimistic:{cpc:.9,ctr:3.1,cr:2.6,expectedROI:85}},i=n[e]||n.realistic,r=Math.round(o/i.cpc),s=Math.round(r*(i.ctr/100)*(i.cr/100)),l=Math.round(s*a),d=l-o,u=Math.round(d/o*100),p={cpc:i.cpc.toFixed(2),ctr:i.ctr.toFixed(1),cr:i.cr.toFixed(1),clicks:r.toString(),conversions:s.toString(),revenue:l.toString(),adSpend:o.toString(),profit:d.toString(),roi:u.toString(),breakeven:d>0?Math.max(5,Math.round(20*o/l)).toString():"45"};return console.log(`✅ Escenario ${e} generado:`,p),p},ensureDifferentScenarios:function(e){console.log("🔍 Validando que los escenarios sean diferentes...");const t=parseFloat(e.conservative.cpc||"0"),o=parseFloat(e.realistic.cpc||"0"),a=parseFloat(e.optimistic.cpc||"0");t===o&&o===a&&(console.log("⚠️ Los escenarios son idénticos, regenerando..."),e.conservative=this.generateFallbackScenario("conservative"),e.realistic=this.generateFallbackScenario("realistic"),e.optimistic=this.generateFallbackScenario("optimistic"),console.log("✅ Escenarios regenerados como diferentes"));const n=parseFloat(e.conservative.cpc);parseFloat(e.realistic.cpc);const i=parseFloat(e.optimistic.cpc);if(n<i){console.log("⚠️ Orden de CPC incorrecto, ajustando...");const r=e.conservative;e.conservative=e.optimistic,e.optimistic=r}console.log("✅ Validación completada - Escenarios son diferentes")},validateCalculationLogic:function(e){console.log("🔍 FORZANDO escenarios ULTRA-DIFERENTES...");const t=parseFloat(document.getElementById("calcBudget").value)||50,o=parseInt(document.getElementById("calcDays").value)||30,a=t*o;let n=5;try{if(this.currentProduct&&this.currentProduct.comision){const s=this.currentProduct.comision.toString();console.log("🔍 Detectando comisión:",s);const l=s.match(/\$?([\d,]+\.?\d*)/);if(l)n=parseFloat(l[1].replace(/,/g,"")),console.log("💰 Comisión en dólares detectada:",n);else{const d=s.match(/(\d+)%/);if(d){const u=parseInt(d[1]),p=this.currentProduct.precio||"$19.99",g=parseFloat(p.replace(/[^0-9.]/g,""))||19.99;n=g*u/100,console.log(`💰 Comisión calculada: ${u}% de $${g} = $${n}`)}}}}catch{console.log("⚠️ Error detectando comisión, usando default:",n)}const i={conservative:{cpc:2.8,ctr:1,cr:.8,multiplier:.3},realistic:{cpc:1.6,ctr:2.2,cr:1.9,multiplier:1},optimistic:{cpc:.75,ctr:3.5,cr:3,multiplier:2.8}};Object.keys(i).forEach(s=>{const l=i[s];e[s]={},e[s].cpc=l.cpc.toFixed(2),e[s].ctr=l.ctr.toFixed(1),e[s].cr=l.cr.toFixed(1);const d=Math.round(a/l.cpc),u=Math.round(d*(l.ctr/100)*(l.cr/100)*l.multiplier),p=Math.round(u*n),g=p-a,v=a>0?Math.round(g/a*100):0,C={conservative:60,realistic:35,optimistic:15};e[s].clicks=d.toString(),e[s].conversions=u.toString(),e[s].revenue=p.toString(),e[s].profit=g.toString(),e[s].roi=v.toString(),e[s].adSpend=a.toString(),e[s].breakeven=C[s].toString(),console.log(`🚀 ${s.toUpperCase()}:`,{cpc:`$${e[s].cpc}`,conversions:e[s].conversions,profit:`$${e[s].profit}`,roi:`${e[s].roi}%`})});const r=parseFloat(e.realistic.profit||"0");if(r<0){const s=Math.abs(r);e.scaling={month1:r.toString(),month2:Math.round(r*.3).toString(),month3:Math.round(s*.4).toString()}}else e.scaling={month1:r.toString(),month2:Math.round(r*1.6).toString(),month3:Math.round(r*2.2).toString()};console.log("📈 Scaling aplicado:",e.scaling),console.log("✅ ESCENARIOS ULTRA-DIFERENTES COMPLETADOS")},generateDefaultRecommendations:function(){return`Basándote en el análisis del producto y configuración:
...
`});const a=this.currentScenarios.scaling||{};o+=`📈 PROYECCIÓN DE ESCALAMIENTO
`,o+=`- Mes 1: $${a.month1||"500"}
`,o+=`- Mes 2: $${a.month2||"1200"}
`,o+=`- Mes 3: $${a.month3||"2500"}

`,o+=`💡 RECOMENDACIONES DE IA
`,o+=this.currentScenarios.recommendations||"No disponibles";const n=new Blob([o],{type:"text/plain"}),i=URL.createObjectURL(n),r=document.createElement("a");r.href=i,r.download=`profit-report-${Date.now()}.txt`,r.click(),URL.revokeObjectURL(i),alert("✅ Reporte exportado exitosamente")}catch(e){console.error("Error exportando reporte:",e),alert("❌ Error al exportar reporte")}},saveScenario:function(){if(!this.currentScenarios){alert("⚠️ No hay escenarios para guardar");return}try{const e=JSON.parse(localStorage.getItem("saved_scenarios")||"[]");e.push({date:new Date().toISOString(),product:this.currentProduct.nombre,scenarios:this.currentScenarios,config:{budget:document.getElementById("calcBudget").value,channel:document.getElementById("calcChannel").value,days:document.getElementById("calcDays").value,market:document.getElementById("calcMarket").value}}),localStorage.setItem("saved_scenarios",JSON.stringify(e)),alert("✅ Escenario guardado exitosamente")}catch(e){console.error("Error guardando escenario:",e),alert("❌ Error al guardar escenario")}}};function W(){console.log("💰 Agregando botones de Profit Calculator..."),document.querySelectorAll(".product-opportunity").forEach((e,t)=>{if(!e.querySelector(".profit-calc-btn")){const o=e.querySelector(".product-actions")||e.querySelector(".spy-btn")?.parentElement||e,a=document.createElement("button");a.className="btn btn-secondary profit-calc-btn",a.innerHTML="💰 Calcular Profit",a.style.marginTop="10px",a.style.marginLeft="10px",a.onclick=()=>{if(m.productosDetectados&&m.productosDetectados[t]){const n=m.productosDetectados[t];H.open(n,t)}else alert("⚠️ Producto no encontrado")},e.querySelector(".spy-btn")?e.querySelector(".spy-btn").after(a):e.querySelector(".validate-btn")?e.querySelector(".validate-btn").after(a):o.appendChild(a)}}),console.log("✅ Botones de Profit Calculator agregados")}if(typeof D<"u"){const e=D;D=function(){try{e(),setTimeout(W,100)}catch(t){console.error("Error en addSpyButtons:",t)}}}const I={generators:{facebook:(e,t)=>{const o=e.painPoints||"",a=e.emociones||"",n=e.triggers||"",i=e.descripcion||"",r=e.estacionalidad||"",s=e.horarioOptimo||"",l=e.competenciaNivel||"",d=e.timingOptimo||"",u=e.estrategia||"",p=o.split(/[,.]/).filter(b=>b.trim()),g=a.split(/[,.]/).filter(b=>b.trim()),v=n.split(/[,.]/).filter(b=>b.trim()),C=p.slice(0,3).map((b,G)=>{const y=b.trim().toLowerCase();return y.includes("falta de tiempo")?"✅ Resultados en solo 15 minutos al día":y.includes("dificultad")?"✅ Método simple paso a paso que cualquiera puede seguir":y.includes("peso")||y.includes("grasa")?"✅ Pierde hasta 2 kilos por semana sin pasar hambre":y.includes("energía")?"✅ Energía ilimitada desde el primer día":y.includes("dinero")||y.includes("caro")?"✅ Inversión mínima con resultados máximos":y.includes("motivación")?"✅ Sistema que te mantiene motivado todos los días":`✅ ${b.trim().replace(/no poder|no lograr|falta de|sin/gi,"Lograrás").replace(/dificultad para|problema con/gi,"Dominarás")}`});C.length===0&&C.push("✅ Resultados visibles desde la primera semana","✅ Método probado por miles de personas","✅ Garantía de satisfacción del 100%");const T=g[0]||"frustración",F=v[0]||"necesidad de cambio",U=[`😱 ¿${T.charAt(0).toUpperCase()+T.slice(1)}? ${e.nombre} es la solución que buscabas`,`🔥 "${p[0]||"Este problema"}" - Si esto te suena familiar, necesitas ${e.nombre}`,`⚠️ ATENCIÓN: ${e.nombre} con ${e.comision||"40% descuento"} (Solo hoy)`,`💥 ${F.charAt(0).toUpperCase()+F.slice(1)}? Descubre cómo ${e.nombre} está cambiando vidas`,`🎯 Por fin: La solución definitiva para ${p[0]||t} está aquí`];return`${U[Math.floor(Math.random()*U.length)]}
...
${n?`Competencia actual: ${n}`:""}
${o?`Estacionalidad: ${o}`:""}`},email:(e,t)=>{const o=e.estacionalidad||"",a=e.horarioOptimo||"",n=e.timingOptimo||"",i=e.competenciaNivel||"",r=e.estrategia||"",s=e.painPoints?e.painPoints.split(/[,.]/).filter(p=>p.trim())[0]:`los desafíos en ${t}`,l=e.emociones?e.emociones.split(",")[0].trim():"frustración",d=e.triggers?e.triggers.split(",")[0].trim():"necesitas una solución real";return`📧 SECUENCIA DE EMAIL DE ALTA CONVERSIÓN
...
${a?`
📱 MEJOR MOMENTO PARA ENVIAR: ${a}`:""}
${i?`
🎯 NIVEL DE COMPETENCIA: ${i} - Ventaja competitiva clara`:""}`}},copyTemplate:async(e,t,o)=>{try{const a=I.generators[e](t,o);if(navigator.clipboard&&navigator.clipboard.writeText)await navigator.clipboard.writeText(a);else{const n=document.createElement("textarea");n.value=a,n.style.position="fixed",n.style.opacity="0",document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}return I.showNotification(`✅ Template ${e.toUpperCase()} copiado (${a.length} caracteres)`,"success"),console.log(`Template ${e} copiado exitosamente`),!0}catch(a){return console.error("Error copiando template:",a),I.showNotification("❌ Error al copiar. Intenta de nuevo.","error"),!1}},showNotification:(e,t="success")=>{document.querySelectorAll(".template-notification").forEach(n=>n.remove());const a=document.createElement("div");a.className=`template-notification ${t}`,a.innerHTML=e,a.style.cssText=`
            position: fixed;
            top: 20px;
            right: 20px;
            padding: 15px 25px;
            ...
            color: white;
            max-width: 400px;
        `,document.body.appendChild(a),setTimeout(()=>{a.style.opacity="0",a.style.transform="translateX(100px)",setTimeout(()=>a.remove(),300)},3e3)},addTemplateButtons:()=>{console.log("Agregando botones de templates..."),document.querySelectorAll(".product-opportunity").forEach((e,t)=>{if(e.querySelector(".template-buttons"))return;if(!m.productosDetectados[t]){console.log(`No hay producto en índice ${t}`);return}const a=document.getElementById("nicho")?.value||"marketing",n=document.createElement("div");n.className="template-buttons",n.style.cssText=`
            ...
            `,n.innerHTML=`
                ...
                <div class="template-buttons-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 10px;">
                    <button class="btn-template facebook" 
                            ...
                            onclick="CopyTemplateSystem.copyTemplate('facebook', AppState.productosDetectados[${t}], '${a.replace(/'/g,"\\'")}')">
                        📘 Facebook Ad
                    </button>
                    <button class="btn-template google" 
                            ...
                            onclick="CopyTemplateSystem.copyTemplate('google', AppState.productosDetectados[${t}], '${a.replace(/'/g,"\\'")}')">
                        🔍 Google Ad
                    </button>
                    <button class="btn-template email" 
                            ...
                            onclick="CopyTemplateSystem.copyTemplate('email', AppState.productosDetectados[${t}], '${a.replace(/'/g,"\\'")}')">
                        📧 Email Sequence
                    </button>
                </div>
                <div class="ai-template-buttons" style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 10px;">
                    <button class="btn-ai-template" 
                            ...
                            onclick="CopyTemplateSystem.generateAITemplate(${t}, '${a.replace(/'/g,"\\'")}')">
                        🤖 IA Específica
                    </button>
                    <button class="btn-ab-template" 
                            ...
                            onclick="CopyTemplateSystem.generateABTemplate(${t}, '${a.replace(/'/g,"\\'")}')">
                        🔄 A/B Testing
                    </button>
                </div>
            `;const i=e.querySelector(".product-actions"),r=e.querySelector(".offer-validation"),s=e.querySelector(".spy-results");r?r.parentNode.insertBefore(n,r):s?s.parentNode.insertBefore(n,s):i?e.insertBefore(n,i):e.appendChild(n)}),console.log("Botones de templates agregados exitosamente")},generateAICopy:async(e,t,o)=>{const a=`
...
IMPORTANTE: Devuelve SOLO el copy optimizado, sin explicaciones.`;try{return await A.callGemini(a)}catch(n){return console.error("Error generando copy con IA:",n),I.generators[o](e,t)}},generateABVariations:async(e,t,o)=>{const a=`
...
=== VARIACIÓN C ===
[Copy completo]`;try{return await A.callGemini(a)}catch(n){return console.error("Error generando variaciones A/B:",n),"Error generando variaciones. Intenta de nuevo."}},generateAITemplate:async(e,t)=>{const o=m.productosDetectados[e];if(!o)return;const a=await I.showTypeSelector();if(!a)return;const n=I.createTemplateModal(`🤖 Generando copy con IA específica para ${a}...`);document.body.appendChild(n);try{const i=await I.generateAICopy(o,t,a);n.querySelector(".modal-content").innerHTML=`
                <div class="modal-header">
                    <h3>🤖 Copy IA Específica - ${a.toUpperCase()}</h3>
                ...
                <div class="modal-body">
                    ...
                    <div class="template-actions" style="margin-top: 15px; display: flex; gap: 10px;">
                        ...
                        <button onclick="CopyTemplateSystem.downloadFromModal(this, '${a}')" class="btn-secondary">💾 Descargar</button>
                        <button onclick="CopyTemplateSystem.regenerateTemplate(${e}, '${t}', '${a}')" class="btn-accent">🔄 Regenerar</button>
                    </div>
                </div>
            `,I.showNotification("✅ Copy IA específica generado!","success")}catch(i){console.error("Error:",i),n.querySelector(".modal-content").innerHTML=`
                <div class="modal-header">
                    <h3>❌ Error</h3>
                    <button class="close-modal" onclick="this.closest('.template-modal').remove()">×</button>
                </div>
            ...
            `}},generateABTemplate:async(e,t)=>{const o=m.productosDetectados[e];if(!o)return;const a=await I.showTypeSelector();if(!a)return;const n=I.createTemplateModal(`🔄 Generando 3 variaciones A/B para ${a}...`);document.body.appendChild(n);try{const i=await I.generateABVariations(o,t,a);n.querySelector(".modal-content").innerHTML=`
                <div class="modal-header">
                    <h3>🔄 Variaciones A/B - ${a.toUpperCase()}</h3>
                    <button class="close-modal" onclick="this.closest('.template-modal').remove()">×</button>
                </div>
                <div class="modal-body">
                    <div class="template-result">
                    ...
                    <div class="template-actions" style="margin-top: 15px; display: flex; gap: 10px;">
                        ...
                        <button onclick="CopyTemplateSystem.downloadFromModal(this, '${a}-ab')" class="btn-secondary">💾 Descargar</button>
                        <button onclick="CopyTemplateSystem.generateABTemplate(${e}, '${t}')" class="btn-accent">🔄 Nuevas Variaciones</button>
                    </div>
                </div>
            `,I.showNotification("✅ 3 variaciones A/B generadas!","success")}catch(i){console.error("Error:",i),n.querySelector(".modal-content").innerHTML=`
                <div class="modal-header">
                    <h3>❌ Error</h3>
                    <button class="close-modal" onclick="this.closest('.template-modal').remove()">×</button>
                </div>
            ...
            `,t.querySelectorAll(".type-btn").forEach(o=>{o.addEventListener("click",()=>{const a=o.dataset.type;t.remove(),e(a)})}),document.body.appendChild(t)}),createTemplateModal:e=>{const t=document.createElement("div");return t.className="template-modal",t.style.cssText=`
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
        ...
        `,t},copyFromModal:async e=>{const t=e.closest(".modal-body").querySelector("textarea");if(t)try{await navigator.clipboard.writeText(t.value),I.showNotification("✅ Copiado al portapapeles!","success")}catch(o){console.error("Error copiando:",o),I.showNotification("❌ Error al copiar","error")}},downloadFromModal:(e,t)=>{const o=e.closest(".modal-body").querySelector("textarea");if(o){const a=new Blob([o.value],{type:"text/plain"}),n=URL.createObjectURL(a),i=document.createElement("a");i.href=n,i.download=`copy-${t}-${Date.now()}.txt`,i.click(),URL.revokeObjectURL(n),I.showNotification("✅ Descargado!","success")}},regenerateTemplate:async(e,t,o)=>{const a=m.productosDetectados[e];if(!a)return;const i=document.querySelector(".template-modal").querySelector(".modal-content");i.innerHTML=`
            <div class="loading-content" style="text-align: center; padding: 40px;">
                <div class="spinner" style="width: 40px; height: 40px; border: 4px solid #f3f4f6; border-top: 4px solid #3b82f6; border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 20px;"></div>
                <p>🔄 Regenerando copy con IA específica...</p>
            </div>
        `;try{const r=await I.generateAICopy(a,t,o);i.innerHTML=`
                <div class="modal-header">
            ...
            `}}},ye=E.displayResults;E.displayResults=function(e){ye.call(this,e),setTimeout(()=>{I.addTemplateButtons()},500)};document.addEventListener("DOMContentLoaded",function(){console.log("✅ Inicializando MarketInsight Pro...");try{const e=document.getElementById("profitCalculatorModal");e&&e.addEventListener("click",function(t){t.target===this&&H.closeModal()}),document.addEventListener("keydown",function(t){if(t.key==="Escape"){const o=document.getElementById("profitCalculatorModal");o&&!o.classList.contains("hidden")&&H.closeModal()}}),console.log("💰 Profit Calculator inicializado")}catch(e){console.error("Error inicializando:",e)}});document.addEventListener("click",function(e){if(e.target.matches(".copy-hook, .copy-audience, .copy-framework-btn")){e.preventDefault();const t=decodeURIComponent(e.target.dataset.textToCopy||"");t&&navigator.clipboard.writeText(t).then(()=>{const o=document.createElement("div");o.className="copy-notification",o.innerHTML="✅ ¡Copiado al portapapeles!",o.style.cssText=`
                    position: fixed;
                    top: ${e.clientY-50}px;
...
Por ahora, usa los hooks y ángulos proporcionados para crear tus propias variantes.`)),e.target.matches(".download-template-btn")){e.preventDefault();const t=e.target.dataset.spyId,o=document.getElementById(t);if(o){const a=o.innerText,n=new Blob([a],{type:"text/plain"}),i=URL.createObjectURL(n),r=document.createElement("a");r.href=i,r.download=`ad-template-${Date.now()}.txt`,r.click(),URL.revokeObjectURL(i),I.showNotification("✅ ¡Template descargado!","success")}}});setInterval(()=>{try{const e=document.querySelectorAll(".product-opportunity");e.length>0&&m.productosDetectados.length>0&&(Array.from(e).some(n=>!n.querySelector(".profit-calc-btn"))&&W(),Array.from(e).some(n=>!n.querySelector(".template-buttons"))&&I.addTemplateButtons(),Array.from(e).some(n=>!n.querySelector(".validate-btn"))&&typeof _<"u"&&_())}catch{}},5e3);console.log("✅ MarketInsight Pro cargado completamente");function Se(){Te(),setTimeout(()=>{const e=document.querySelector(".advanced-config-section");e&&(e.style.display="none",console.log("🔧 Configuración avanzada oculta temporalmente"))},500)}function Te(){const e=document.getElementById("themeToggle"),t=document.body;if(!e)return;const o=localStorage.getItem("theme")||"dark";t.setAttribute("data-theme",o),e.addEventListener("click",()=>{const n=t.getAttribute("data-theme")==="dark"?"light":"dark";t.setAttribute("data-theme",n),localStorage.setItem("theme",n),t.style.transition="all 0.3s ease",setTimeout(()=>{t.style.transition=""},300),c.log(`🎨 Tema cambiado a: ${n}`)})}typeof h<"u"&&(h.generateAdditionalProductsAdvanced=function(e){const t=window.advancedConfig||{productCount:3,minScore:70,activeFilters:["BAJO","MEDIO","ALTO"]},o=t.productCount-e;if(o<=0)return[];const a=[],n=document.getElementById("nicho")?.value||"marketing",i=document.getElementById("publico")?.value||"audiencia",r=[`Curso Avanzado de ${n}`,`Masterclass Completa de ${n}`,`Sistema Premium de ${n}`,`Guía Definitiva de ${n}`,`Entrenamiento VIP de ${n}`,`Blueprint de ${n}`,`Manual Profesional de ${n}`,`Certificación en ${n}`,`Mentoring de ${n}`,`Toolkit de ${n}`];for(let s=0;s<o&&s<r.length;s++){const l=Math.floor(Math.random()*(95-t.minScore))+t.minScore,d=t.activeFilters[Math.floor(Math.random()*t.activeFilters.length)];a.push({nombre:r[s],precio:h.extractRandomPrice(),comision:h.extractRandomCommission(),score:l,descripcion:`Producto profesional de ${n} dirigido a ${i}`,painPoints:[`Falta de conocimiento en ${n}`,"Necesidad de resultados rápidos","Búsqueda de métodos probados"],emociones:["Frustración","Esperanza","Determinación"],triggers:["Exclusividad","Resultados garantizados","Soporte premium"],competencia:d,networks:["ClickBank","ShareASale","CJ Affiliate"],tips:[`Enfocarse en ${i} específicamente`,"Usar testimonios reales","Crear urgencia con tiempo limitado"]})}return c.log(`✅ Generados ${a.length} productos adicionales con score mínimo ${t.minScore}`),a});document.addEventListener("DOMContentLoaded",function(){setTimeout(()=>{Se(),c.log("✅ Funcionalidades avanzadas inicializadas")},1e3)});console.log("✅ Funcionalidades profesionales cargadas");typeof window<"u"&&(window.APIManager=A,window.AppState=m,window.Utils=c);console.log("MarketInsight Pro (Vite + TS) initialized");const Y={gemini:{name:"Google Gemini",keyLink:"https://aistudio.google.com/app/apikey",request:async()=>{throw new Error("Gemini request debe ser manejada por la implementación original")}},openai:{name:"OpenAI GPT-3.5",keyLink:"https://platform.openai.com/account/api-keys",request:async(e,t)=>{const o=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify({model:"gpt-3.5-turbo",messages:[{role:"system",content:"Follow the user instructions strictly and use the exact JSON-like structure requested. Do NOT add any additional commentary."},{role:"user",content:e}],temperature:.4,max_tokens:4e3})});if(!o.ok){const n=await o.text();throw new Error(`OpenAI error ${o.status}: ${n}`)}return(await o.json()).choices?.[0]?.message?.content||""}},together:{name:"Together.ai (Mistral 7B)",keyLink:"https://app.together.xyz/settings/api-keys",request:async(e,t)=>{const o=await fetch("https://api.together.xyz/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify({model:"mistral-7b-instruct",messages:[{role:"system",content:"Follow the user instructions strictly and use the exact JSON-like structure requested. Do NOT add any additional commentary."},{role:"user",content:e}],temperature:.4,max_tokens:4e3})});if(!o.ok){const n=await o.text();throw new Error(`Together.ai error ${o.status}: ${n}`)}const a=await o.json();return(a.text||a.response||a.generations?.[0]?.text)??""}},cohere:{name:"Cohere (Command-R)",keyLink:"https://dashboard.cohere.com/api-keys",request:async(e,t)=>{const o=await fetch("https://api.cohere.ai/v1/chat",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify({model:"command-r",message:e,temperature:.4,max_tokens:4e3})});if(!o.ok){const n=await o.text();throw new Error(`Cohere error ${o.status}: ${n}`)}const a=await o.json();return(a.text||a.response||a.generations?.[0]?.text)??""}}};function j(){const e=document.getElementById("apiProvider"),t=document.getElementById("getKeyLink");if(!e)return;const o=localStorage.getItem("aiProvider")||"gemini";e.value=o,window.AppState.apiProvider=o;const a=()=>{const n=e.value;if(window.AppState.apiProvider=n,localStorage.setItem("aiProvider",n),t){const i=Y[n];t.href=i.keyLink,t.textContent=`Obtener API Key (${i.name})`}};e.addEventListener("change",a),a()}function K(){const e=window;if(!e.APIManager)return;const t=e.APIManager.callGemini.bind(e.APIManager);e.APIManager.callGemini=async o=>{const a=e.AppState?.apiProvider||"gemini";if(a==="gemini")return t(o);const n=Y[a];if(!n)throw new Error(`Proveedor no soportado: ${a}`);return await n.request(o,e.AppState.apiKey)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{j(),K(),X()}):(j(),K(),X());function X(){const e=window;e.Utils&&(e.Utils.validateApiKey=t=>{const o=e.AppState?.apiProvider||"gemini";return!t||t.trim().length===0?{valid:!1,message:"API Key vacía"}:o==="gemini"?t.length<20||!t.startsWith("AIza")?{valid:!1,message:"Formato de API Key inválido para Google AI Studio"}:{valid:!0,message:"API Key válida"}:t.length<20?{valid:!1,message:"API Key muy corta"}:{valid:!0,message:"API Key válida"}})}
