...
[Copy completo]`;try{return await C.callGemini(n)}catch(a){return console.error("Error generando variaciones A/B:",a),"Error generando variaciones. Intenta de nuevo."}},generateAITemplate:async(e,t)=>{const o=m.productosDetectados[e];if(!o)return;const n=await I.showTypeSelector();if(!n)return;const a=I.createTemplateModal(`🤖 Generando copy con IA específica para ${n}...`);document.body.appendChild(a);try{const i=await I.generateAICopy(o,t,n);a.querySelector(".modal-content").innerHTML=`
        ...
        `;try{const r=await I.generateAICopy(n,t,o);i.innerHTML=`
            ...
            `}}},Se=E.displayResults;E.displayResults=function(e){Se.call(this,e),setTimeout(()=>{I.addTemplateButtons()},500)};document.addEventListener("DOMContentLoaded",function(){console.log("✅ Inicializando MarketInsight Pro...");try{const e=document.getElementById("profitCalculatorModal");e&&e.addEventListener("click",function(t){t.target===this&&U.closeModal()}),document.addEventListener("keydown",function(t){if(t.key==="Escape"){const o=document.getElementById("profitCalculatorModal");o&&!o.classList.contains("hidden")&&U.closeModal()}}),console.log("💰 Profit Calculator inicializado")}catch(e){console.error("Error inicializando:",e)}});document.addEventListener("click",function(e){if(e.target.matches(".copy-hook, .copy-audience, .copy-framework-btn")){e.preventDefault();const t=decodeURIComponent(e.target.dataset.textToCopy||"");t&&navigator.clipboard.writeText(t).then(()=>{const o=document.createElement("div");o.className="copy-notification",o.innerHTML="✅ ¡Copiado al portapapeles!",o.style.cssText=`
                    position: fixed;
                    top: ${e.clientY-50}px;
