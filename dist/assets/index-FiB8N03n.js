...
- Tipo conversión: ${p}
- Rango precio productos: ${W}
- Tipo producto: ${a}

🎯 ANÁLISIS REQUERIDOS: ${[C&&"Competencia",S&&"Tendencias",q&&"Estacionalidad",G&&"Conversión",y&&"Financiero",X&&"Intel Competitiva",J&&"Canales Tráfico"].filter(Boolean).join(", ")}`;return`Actúa como CONSULTOR EXPERTO en marketing de afiliados especializado en ${t} para ${i} en ${v} con 15+ años detectando productos ganadores.
...
Haz este avatar TAN específico que cualquier marketer pueda hablarle directamente y convertir 3-5x más.`,i=await A.callGemini(a);se(i),c.showStatus("✅ Avatar creado exitosamente","success")}catch(a){console.error("Error:",a),c.showStatus(`❌ Error: ${a.message}`,"error")}finally{o.innerHTML=n,o.disabled=!1}}function ie(e,t,o,n=null){let a=document.getElementById("contentResults");if(a||(a=document.createElement("div"),a.id="contentResults",a.className="content-results",document.querySelector(".main-content").appendChild(a)),a.innerHTML=`
        ...
        <div class="content-display">
            <div class="content-item">
                <div class="content-title" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                    📱 Contenido Generado para: ${t.map(i=>re(i)+" "+i.toUpperCase()).join(", ")}
                </div>
    ...
    `,t.classList.remove("hidden"),t.scrollIntoView({behavior:"smooth"}),window.lastAvatarGenerated=e}function k(){console.log("Inicializando nuevas funcionalidades..."),setTimeout(te,100);const e=document.getElementById("generateContentBtn"),t=document.getElementById("generateAvatarBtn");e&&(e.onclick=ae,console.log("Botón contenido configurado")),t&&(t.onclick=ne,console.log("Botón avatar configurado")),console.log("Nuevas funcionalidades inicializadas correctamente")}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",k):k();setTimeout(k,500);setTimeout(k,1500);async function ce(){if(console.log("Generando múltiples avatares automáticamente..."),!m.apiKey){alert("⚠️ Configura tu API Key primero");return}const e={nicho:document.getElementById("nicho").value.trim(),publico:document.getElementById("publico").value.trim(),tipoProducto:document.getElementById("tipoProducto").value,rangoPrecios:document.getElementById("rangoPrecios").value,canalPrincipal:document.getElementById("canalPrincipal").value,presupuestoAds:document.getElementById("presupuestoAds").value,roiObjetivo:document.getElementById("roiObjetivo").value,mercadoGeo:document.getElementById("mercadoGeo").value,dispositivoTarget:document.getElementById("dispositivoTarget").value,experiencia:document.getElementById("experiencia").value};if(!e.nicho||!e.publico){alert("⚠️ Completa el nicho y público objetivo primero");return}const t=document.getElementById("generateMultipleAvatarsBtn"),o=t.innerHTML;t.innerHTML="🤖 Generando 5 Avatares...",t.disabled=!0;try{const n=le(e),a=await A.callGemini(n);de(a),c.showStatus("✅ 5 avatares generados automáticamente","success")}catch(n){console.error("Error:",n),c.showStatus(`❌ Error: ${n.message}`,"error")}finally{t.innerHTML=o,t.disabled=!1}}function le(e){const{nicho:t,publico:o,tipoProducto:n,rangoPrecios:a,canalPrincipal:i,presupuestoAds:r,roiObjetivo:s,mercadoGeo:l,dispositivoTarget:d}=e;return`Actúa como EXPERTO EN SEGMENTACIÓN DE AUDIENCIAS con 15+ años creando avatares ultra-específicos.
...
OBJETIVO: 5 avatares TAN específicos que puedas crear campañas ultra-dirigidas para cada uno con mensajes completamente diferentes.`}function de(e){let t=document.getElementById("multipleAvatarsResults");t||(t=document.createElement("div"),t.id="multipleAvatarsResults",t.className="multiple-avatars-results",document.querySelector(".main-content").appendChild(t));const o=pe(e);let n=`
    ...
    `,t.innerHTML=n,t.classList.remove("hidden"),t.scrollIntoView({behavior:"smooth"}),window.lastMultipleAvatars=e,window.processedAvatars=o}function pe(e){const t=[],o=/=== AVATAR \d+: ([^=]+) ===([\s\S]*?)(?==== AVATAR \d+:|$)/g;let n;for(;(n=o.exec(e))!==null;)t.push({titulo:n[1].trim(),contenido:n[2].trim()});return t.length===0&&e.split(/AVATAR \d+:/).forEach((i,r)=>{i.trim()&&r>0&&t.push({titulo:`Avatar ${r}`,contenido:i.trim()})}),t}function me(){const e=document.getElementById("generateAvatarBtn");if(e&&!document.getElementById("generateMultipleAvatarsBtn")){const t=document.createElement("button");t.className="btn btn-avatar",t.id="generateMultipleAvatarsBtn",t.style.marginTop="15px",t.innerHTML='<span class="btn-icon">🤖</span><span class="btn-text">Generar 5 Avatares Automáticamente</span>',t.onclick=ce,e.parentNode.appendChild(t)}}const ue=`
...
`;function ge(){if(!document.getElementById("multipleAvatarsCSS")){const e=document.createElement("style");e.id="multipleAvatarsCSS",e.textContent=ue,document.head.appendChild(e)}}function V(){ge(),setTimeout(me,1e3),console.log("Auto-generador de avatares múltiples inicializado")}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",V):V();setTimeout(V,1e3);const z=window.mostrarResultadosAvatar||function(){};E.displayResults;window.mostrarResultadosAvatar=function(e){window.lastAvatarGenerated=e,typeof z=="function"&&z(e),setTimeout(updateFunnelExportButton,500),console.log("✅ Avatar guardado globalmente")};if(typeof E<"u"&&E.displayResults){const e=E.displayResults;E.displayResults=function(t){t&&t.productos&&(typeof m>"u"&&(window.AppState={}),m.productosDetectados=t.productos,console.log("✅ Productos guardados globalmente:",t.productos.length)),e.call(this,t),setTimeout(updateFunnelExportButton,500)}}setInterval(function(){const e=!!window.lastAvatarGenerated,t=!!(typeof m<"u"&&m.productosDetectados&&m.productosDetectados.length>0);(e||t)&&updateFunnelExportButton&&updateFunnelExportButton()},2e3);document.addEventListener("DOMContentLoaded",function(){setTimeout(function(){const e=document.getElementById("avatarResults"),t=document.getElementById("resultados");if(e&&!e.classList.contains("hidden")){console.log("🔍 Avatar detectado en pantalla, buscando datos...");const o=e.querySelector(".avatar-content, .avatar-display");o&&o.textContent&&(window.lastAvatarGenerated=o.textContent,console.log("✅ Avatar recuperado del DOM"))}t&&!t.classList.contains("hidden")&&(console.log("🔍 Productos detectados en pantalla, simulando datos..."),typeof m>"u"&&(window.AppState={}),m.productosDetectados||(m.productosDetectados=[{nombre:"Producto detectado",precio:"$50-200",descripcion:"Producto de fitness y bienestar"}],console.log("✅ Productos simulados"))),updateFunnelExportButton&&updateFunnelExportButton()},1e3)});console.log("🔧 Fix de variables globales cargado");const P={openTrendPredictor:()=>{console.log("🔮 Abriendo Trend Predictor...");const e={nicho:document.getElementById("nicho")?.value?.trim()||"",mercado:document.getElementById("mercadoGeo")?.value||"LATAM",tipoProducto:document.getElementById("tipoProducto")?.value||"digital",canalPrincipal:document.getElementById("canalPrincipal")?.value||"paid",presupuestoAds:document.getElementById("presupuestoAds")?.value||"1000",experiencia:document.getElementById("experiencia")?.value||"intermedio"};if(!e.nicho){alert("⚠️ Ingresa un nicho primero");return}localStorage.setItem("main_nicho",e.nicho),localStorage.setItem("main_mercado",e.mercado),localStorage.setItem("main_config",JSON.stringify(e));const o=`trend-predictor.html?${new URLSearchParams({nicho:e.nicho,mercado:e.mercado,source:"marketinsight-pro"}).toString()}`;window.open(o,"_blank","width=1400,height=900,scrollbars=yes,resizable=yes")?console.log("✅ Trend Predictor abierto exitosamente"):alert("⚠️ Permitir pop-ups para abrir Trend Predictor"),typeof c<"u"&&c.showStatus&&c.showStatus(`🔮 Trend Predictor abierto para: ${e.nicho}`,"success"),console.log("🔮 Configuración enviada:",e)},canUseTrendPredictor:()=>{const e=localStorage.getItem("gemini_api_key"),t=document.getElementById("nicho")?.value?.trim();return!!(e&&t)},updateTrendButton:()=>{const e=document.getElementById("openTrendPredictorBtn");if(!e)return;P.canUseTrendPredictor();const t=document.getElementById("nicho")?.value?.trim()||"";localStorage.getItem("gemini_api_key")?t?(e.style.opacity="1",e.disabled=!1,e.innerHTML=`🔮 Predecir Tendencias: ${t}`):(e.style.opacity="0.6",e.disabled=!0,e.innerHTML="🔮 Trend Predictor (Ingresa nicho primero)"):(e.style.opacity="0.6",e.disabled=!0,e.innerHTML="🔮 Trend Predictor (Configura API Key primero)")}};function j(){const e=document.getElementById("generateBtn");if(!e){setTimeout(j,1e3);return}if(document.getElementById("openTrendPredictorBtn"))return;console.log("📋 Agregando botón Trend Predictor...");const t=document.createElement("button");t.id="openTrendPredictorBtn",t.className="btn btn-primary",t.style.cssText=`
    ...
    `,t=document.createElement("style");t.id="trendPredictorStyles",t.textContent=e,document.head.appendChild(t),console.log("🎨 Estilos Trend Predictor agregados")}function w(){console.log("🔮 Inicializando integración Trend Predictor..."),ve(),setTimeout(j,1e3),setTimeout(Ee,1500),new URLSearchParams(window.location.search).get("from")==="trend-predictor"&&(console.log("🔄 Usuario regresando desde Trend Predictor"),typeof c<"u"&&c.showStatus&&c.showStatus("🔮 Datos de tendencias disponibles para análisis","info")),console.log("✅ Integración Trend Predictor inicializada completamente")}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",w):w();setTimeout(w,2e3);setTimeout(()=>{document.getElementById("openTrendPredictorBtn")||(console.log("🔄 Ejecutando respaldo de inicialización..."),w())},4e3);function fe(){console.log("🔧 DEBUG TREND PREDICTOR:"),console.log("- API Key:",!!localStorage.getItem("gemini_api_key")),console.log("- Nicho campo:",document.getElementById("nicho")?.value||"NO ENCONTRADO"),console.log("- Botón existe:",!!document.getElementById("openTrendPredictorBtn")),console.log("- Estilos cargados:",!!document.getElementById("trendPredictorStyles"));const e=document.getElementById("openTrendPredictorBtn");e&&(console.log("- Botón habilitado:",!e.disabled),console.log("- Texto del botón:",e.textContent))}window.debugTrendPredictor=fe;console.log("🔮 Trend Predictor Integration cargado. Usa debugTrendPredictor() para troubleshooting.");const $={validateOffer:async(e,t)=>{if(!m.apiKey){alert("⚠️ Configura tu API Key primero");return}const o=`Actúa como SUPER AFILIADO EXPERTO con 15+ años en ClickBank, ShareASale, CJ, MaxBounty y acceso a datos internos de networks.
